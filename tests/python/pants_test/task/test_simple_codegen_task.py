...
class DummyGen(SimpleCodegenTask):
    ...
    def execute_codegen(self, target, target_workdir):
        ...
        for path in self._dummy_sources_to_generate(target, target_workdir):
            ...
            package_name = os.path.relpath(os.path.dirname(path), target_workdir).replace(
                os.path.sep, "."
            )
...
class SimpleCodegenTaskTest(TaskTestBase):
    ...
    def alias_groups(cls):
        return register_core().merge(BuildFileAliases({"dummy_library": DummyLibrary}))
    ...
    def test_expected_codegen_targets_and_right_execution_count(self):
        ...
        for suffix in dummy_suffixes:
            self.create_file(
                "gen-lib/org/pantsbuild/example/foo{suffix}.dummy".format(suffix=suffix),
                "org.pantsbuild.example Foo{0}".format(suffix),
            )

        targets = [
            self.target("gen-lib:{suffix}".format(suffix=suffix)) for suffix in dummy_suffixes
        ]
        ...
        self.assertEqual(
            expected_targets,
            found_targets,
            "TestGen failed to find codegen target {expected}! Found: [{found}].".format(
                expected=", ".join(t.id for t in expected_targets),
        ...
        self.assertEqual(
            ...
            task.execution_counts,
            "had the wrong number of executions!\n  expected: {}\n  got: {}".format(
                expected_execution_count, task.execution_counts
            ),
        )
    ...
    def _get_duplication_test_targets(self):
        ...
        self.create_file(
            "gen-parent/org/pantsbuild/example/parent.dummy", "org.pantsbuild.example ParentClass"
        )

        self.create_file(
            "gen-child/org/pantsbuild/example/good-child.dummy", "org.pantsbuild.example ChildClass"
        )

        self.create_file(
            "gen-child/org/pantsbuild/example/bad-child.dummy",
            "org.pantsbuild.example ParentClass\n" "org.pantsbuild.example ChildClass",
        )

        return (
            self.target("gen-parent"),
            self.target("gen-child:good"),
            self.target("gen-child:bad"),
        )
    ...
    def _do_test_duplication(self, targets, allow_dups, should_fail):
        ...
        if should_fail:
            ...
            for item in should_not_contain:
                self.assertFalse(
                    item in message, 'Error message should not contain "{}".'.format(item)
                )
        else:
    ...
    def test_copy_target_attributes(self):
        self.create_file("fleem/org/pantsbuild/example/fleem.dummy", "org.pantsbuild.example Fleem")
    ...
    def test_invalidation_of_generated_sources(self):
        ...
        t1 = self.make_target(
            spec="src/dummy/com/foo:one", target_type=DummyLibrary, sources=["one.dummy"]
        )
        ...
        gen_targets = [
            self.build_graph.get_target(syn_addr)
...
class ExportingDummyGen(DummyGen):
    ...
    def synthetic_target_extra_dependencies(self, target, target_workdir):
        return [
            self.context.build_graph.get_target_from_spec(s) for s in self.synthetic_dependencies
        ]
...
class ExportSimpleCodegenTaskTest(TaskTestBase):
    ...
    def test_additional_extra_dependency_fine(self):
        ...
        self.assertEqual(
            self.synthetic_target_for("fleem").dependencies[0].address.spec, "marionette:no-strings"
        )
