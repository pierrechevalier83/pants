...
class TestRunnerTaskMixinGracefulTimeoutTest(TaskTestBase):
    ...
    def test_graceful_kill_if_terminate_expires(self):
        self.process_handler = self.create_process_handler(
            poll_returns=[None], wait_succeeds=[False, False]
        )
        self.set_options(timeouts=True)
        task = self.create_task(self.context())
        ...
        self.assertEqual(
            self.process_handler.call_list,
            [
    ...
    def test_graceful_terminate(self):
        self.process_handler = self.create_process_handler(
            poll_returns=[None], wait_succeeds=[False, True]
        )
...
class TestRunnerTaskMixinXmlParsing(TestRunnerTaskMixin, TestCase):
    ...
    def test_parse_test_info_all_testcases(self):
        with temporary_dir() as xml_dir:
            ...
            self.assertEqual(
                {
                    "testOK": {"result_code": "success", "time": 1.290},
                    "testFailure": {"result_code": "failure", "time": 0.27},
                    "testError": {"result_code": "error", "time": 0.932},
                    "testSkipped": {"result_code": "skipped", "time": 0.1},
                },
                tests_info,
            )
    ...
    def test_parse_test_info_with_missing_attributes(self):
        with temporary_dir() as xml_dir:
            ...
            self.assertEqual(
                {
                    "testOK": {"result_code": "success", "time": None},
                    "": {"result_code": "failure", "time": 0.27},
                    "testSkipped": {"result_code": "skipped", "time": 0.1},
                },
                tests_info,
            )
    ...
    def test_parse_test_info_error_continue(self):
        with temporary_dir() as xml_dir:
            ...
            self.assertEqual({"testError": {"result_code": "error", "time": 1.2}}, tests_info)
