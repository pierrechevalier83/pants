...
class OptionsTest(TestBase):
    ...
    def _parse(
        self, args_str, env=None, config=None, bootstrap_option_values=None, options_cls=None
    ):
        args = shlex.split(str(args_str))
        options_type = options_cls or Options
        options = options_type.create(
            env=env or {},
    ...
    def test_arg_scoping(self):
        ...
        options = self._parse(
            "./pants --verbose compile --no-verbose compile.java -z test " "test.junit --no-verbose"
        )
        self.assertEqual(True, options.for_global_scope().verbose)
        ...
        options = self._parse("./pants --listy='[1, 2]'", config={"DEFAULT": {"listy": "[3, 4]"}})
        ...
        with temporary_file_path() as fp1:
            with temporary_file_path() as fp2:
                options = self._parse(
                    './pants --file-listy="{}" --file-listy="{}"'.format(fp1, fp2)
                )
    ...
    def test_boolean_set_option(self):
        options = self._parse(
            "./pants --store-true-flag --store-false-flag "
    ...
    def test_boolean_negate_option(self):
        options = self._parse(
            "./pants --no-store-true-flag --no-store-false-flag "
    ...
    def test_boolean_invalid_value(self):
        with self.assertRaises(Parser.BooleanConversionError):
            self._parse("./pants", config={"DEFAULT": {"store_true_flag": 11,}}).for_global_scope()
        with self.assertRaises(Parser.BooleanConversionError):
            self._parse(
                "./pants", config={"DEFAULT": {"store_true_flag": "AlmostTrue",}}
            ).for_global_scope()
    ...
    def test_list_option(self):
        ...
        check(
            [4], "./pants", env={"PANTS_GLOBAL_LISTY": "[4]"}, config={"GLOBAL": {"listy": "-[4]"}}
        )
    ...
    def test_dict_list_option(self):
        ...
        # Replacing the default.
        check([{"d": 4, "e": 5}, {"f": 6}], './pants --dict-listy=\'[{"d": 4, "e": 5}, {"f": 6}]\'')

        # Parsing env var correctly.
        check(
            [{"a": 1, "b": 2}, {"c": 3}, {"d": 4, "e": 5}],
            "./pants",
            env={"PANTS_GLOBAL_DICT_LISTY": '{"d": 4, "e": 5}'},
        )
        check(
            [{"a": 1, "b": 2}, {"c": 3}, {"d": 4, "e": 5}, {"f": 6}],
            "./pants",
            env={"PANTS_GLOBAL_DICT_LISTY": '+[{"d": 4, "e": 5}, {"f": 6}]'},
        )
        check(
            [{"d": 4, "e": 5}, {"f": 6}],
            "./pants",
        ...
        check(
            [{"a": 1, "b": 2}, {"c": 3}, {"d": 4, "e": 5}],
            "./pants",
            config={"GLOBAL": {"dict_listy": '{"d": 4, "e": 5}'}},
        )
        check(
            [{"a": 1, "b": 2}, {"c": 3}, {"d": 4, "e": 5}, {"f": 6}],
            "./pants",
            config={"GLOBAL": {"dict_listy": '+[{"d": 4, "e": 5}, {"f": 6}]'}},
        )
        check(
            [{"d": 4, "e": 5}, {"f": 6}],
            "./pants",
    ...
    def test_target_list_option(self):
        ...
        # Appending to the default.
        check(["//:a", "//:b"], "./pants")
        check(["//:a", "//:b", "//:c", "//:d"], "./pants --target-listy=//:c --target-listy=//:d")
        check(["//:a", "//:b", "//:c", "//:d"], './pants --target-listy=\'+["//:c", "//:d"]\'')

        # Replacing the default.
        check(["//:c", "//:d"], './pants --target-listy=\'["//:c", "//:d"]\'')

        # Parsing env var correctly.
        check(["//:a", "//:b", "//:c"], "./pants", env={"PANTS_GLOBAL_TARGET_LISTY": "//:c"})
        check(
            ["//:a", "//:b", "//:c", "//:d"],
            "./pants",
            env={"PANTS_GLOBAL_TARGET_LISTY": '+["//:c", "//:d"]'},
        )
        check(["//:c", "//:d"], "./pants", env={"PANTS_GLOBAL_TARGET_LISTY": '["//:c", "//:d"]'})

        # Parsing config value correctly.
        check(["//:a", "//:b", "//:c"], "./pants", config={"GLOBAL": {"target_listy": "//:c"}})
        check(
            ["//:a", "//:b", "//:c", "//:d"],
            "./pants",
            config={"GLOBAL": {"target_listy": '+["//:c", "//:d"]'}},
        )
        check(["//:c", "//:d"], "./pants", config={"GLOBAL": {"target_listy": '["//:c", "//:d"]'}})
    ...
    def test_dict_option(self):
        ...
        check(
            {"a": "b+", "c": "d"}, "./pants", config={"GLOBAL": {"dicty": '+{"a": "b+", "c": "d"}'}}
        )
        check(
            {"a": "b++", "c": "d"},
            './pants --dicty=\'+{"a": "b++"}\'',
    ...
    def test_frozen_registration(self):
        options = Options.create(
            args=[], env={}, config=self._create_config({}), known_scope_infos=[task("foo")]
        )
    ...
    def test_designdoc_example(self):
        ...
        options = self._parse(
            "./pants --a=1 compile --b=2 compile.java --a=3 --c=4", env=env, config=config
        )
    ...
    def test_file_spec_args(self):
        with temporary_file(binary_mode=False) as tmp:
            ...
            cmdline = (
                './pants --target-spec-file={filename} --pants-config-files="[]" '
                "compile morx:tgt fleem:tgt".format(filename=tmp.name)
            )
    ...
    def test_delayed_deprecated_option(self):
        with self.warnings_catcher() as w:
            delayed_deprecation_option_value = (
                self._parse("./pants --global-delayed-deprecated-option=xxx")
                .for_global_scope()
        ...
        with self.warnings_catcher() as w:
            delayed_passed_option_value = (
                self._parse("./pants --global-delayed-but-already-passed-deprecated-option=xxx")
                .for_global_scope()
    ...
    def test_mutually_exclusive_options_flags(self):
        ...
        with self.assertRaises(Parser.MutuallyExclusiveOptionError):
            self._parse(
                "./pants --mutex-foo=foo --mutex-bar=bar --mutex-baz=baz"
            ).for_global_scope()
    ...
    def test_mutually_exclusive_options_mix(self):
        with self.assertRaises(Parser.MutuallyExclusiveOptionError):
            self._parse(
                "./pants --mutex-foo=foo", env={"PANTS_MUTEX_BAR": "bar"}
            ).for_global_scope()
        ...
        with self.assertRaises(Parser.MutuallyExclusiveOptionError):
            self._parse(
                "./pants stale --mutex-a=foo", env={"PANTS_STALE_MUTEX_B": "bar"}
            ).for_scope("stale")

        with self.assertRaises(Parser.MutuallyExclusiveOptionError):
            self._parse(
                "./pants stale --crufty-new=foo", env={"PANTS_STALE_CRUFTY_OLD": "bar"}
            ).for_scope("stale")

        with self.assertRaises(Parser.MutuallyExclusiveOptionError):
            self._parse(
                "./pants stale --mutex-a=foo", config={"stale": {"mutex_b": "bar"}}
            ).for_scope("stale")

        with self.assertRaises(Parser.MutuallyExclusiveOptionError):
            self._parse(
                "./pants stale --crufty-old=foo", config={"stale": {"crufty_new": "bar"}}
            ).for_scope("stale")
    ...
    def test_middle_scoped_options(self):
        ...
        options = self._parse("./pants", config={"DEFAULT": {"a": 100}, "compile": {"a": 99},})
        self.assertEqual(100, options.for_global_scope().a)
        self.assertEqual(99, options.for_scope("compile").a)
        self.assertEqual(99, options.for_scope("compile.java").a)

        options = self._parse("./pants", env={"PANTS_A": "100", "PANTS_COMPILE_A": "99"})
        self.assertEqual(100, options.for_global_scope().a)
        self.assertEqual(99, options.for_scope("compile").a)
        self.assertEqual(99, options.for_scope("compile.java").a)

        # Command line has precedence over config.
        options = self._parse("./pants compile --a=99", config={"DEFAULT": {"a": 100},})
        self.assertEqual(100, options.for_global_scope().a)
        self.assertEqual(99, options.for_scope("compile").a)
        self.assertEqual(99, options.for_scope("compile.java").a)

        # Command line has precedence over environment.
        options = self._parse("./pants compile --a=99", env={"PANTS_A": "100"},)
        self.assertEqual(100, options.for_global_scope().a)
        self.assertEqual(99, options.for_scope("compile").a)
        self.assertEqual(99, options.for_scope("compile.java").a)
        ...
        options = self._parse(
            "./pants ", config={"DEFAULT": {"a": 100},}, env={"PANTS_COMPILE_A": "99"},
        )
        self.assertEqual(100, options.for_global_scope().a)
        self.assertEqual(99, options.for_scope("compile").a)
        self.assertEqual(99, options.for_scope("compile.java").a)
        ...
        options = self._parse("./pants --a=100", env={"PANTS_COMPILE_A": "99"},)
        self.assertEqual(100, options.for_global_scope().a)
        ...
        options = self._parse("./pants --a=100 ", config={"compile": {"a": 99},})
        self.assertEqual(100, options.for_global_scope().a)
        self.assertEqual(100, options.for_scope("compile").a)
        self.assertEqual(100, options.for_scope("compile.java").a)
        ...
        options = self._parse(
            "./pants --a=100 ", config={"compile": {"a": 99},}, env={"PANTS_A": "100"},
        )
        self.assertEqual(100, options.for_global_scope().a)
        self.assertEqual(100, options.for_scope("compile").a)
        self.assertEqual(100, options.for_scope("compile.java").a)
    ...
    def test_get_fingerprintable_for_scope_ignore_passthru(self):
        ...
        options = self._parse(
            "./pants --store-true-flag --num=88 compile.scala --num=77 "
    ...
    def test_fromfile_config(self):
        def parse_func(dest, fromfile):
            return self._parse(
                "./pants fromfile", config={"fromfile": {dest: "@{}".format(fromfile)}}
            )
    ...
    def test_invalid_option_errors(self):
        def parse_joined_command_line(*args):
            ...
            return self._parse(
                safe_shlex_join(["./pants"] + list(args)),
    ...
    def test_double_registration(self):
        ...
        self.assertRaises(
            OptionAlreadyRegistered, lambda: options.register(GLOBAL_SCOPE, "--foo-bar")
        )
