...
class BootstrapOptionsTest(unittest.TestCase):
    ...
    def test_bootstrap_option_values(self):
        ...
        self._do_test(
            [br(".pants.d"), br("build-support"), br("dist")], config=None, env={}, args=[]
        )

        # Check getting values from config, env and args.
        self._do_test(
            ["/from_config/.pants.d", br("build-support"), br("dist")],
            config={"pants_workdir": "/from_config/.pants.d"},
            env={},
            args=[],
        )
        self._do_test(
            [br(".pants.d"), "/from_env/build-support", br("dist")],
            config=None,
            env={"PANTS_SUPPORTDIR": "/from_env/build-support"},
            args=[],
        )
        self._do_test(
            [br(".pants.d"), br("build-support"), "/from_args/dist"],
            config={},
            env={},
        ...
        self._do_test(
            ["/from_config/.pants.d", "/from_env/build-support", "/from_args/dist"],
            config={
    ...
    def test_bootstrap_bool_option_values(self):
        ...
        self._test_bootstrap_options(
            config={}, env={"PANTS_PANTSRC": "False"}, args=[], pantsrc=False
        )
    ...
    def test_create_bootstrapped_multiple_pants_config_files(self):
        def create_options_bootstrapper(*config_paths):
            return OptionsBootstrapper.create(
                args=["--pants-config-files={}".format(cp) for cp in config_paths]
            )
    ...
    def test_options_pantsrc_files(self):
        def create_options_bootstrapper(*config_paths):
            return OptionsBootstrapper.create(
                args=["--pantsrc-files={}".format(cp) for cp in config_paths]
            )
    ...
    def test_bootstrap_options_explicit_config_path(self):
        ...
        self.assertEqual(
            ["{}/pants.ini".format(get_buildroot()), "/from/env", "/from/flag"],
            config_path(
                "main",
                "args",
                "-x",
    ...
    def test_setting_pants_config_in_config(self):
        ...
        with temporary_dir() as tmpdir:
            ...
            ob = OptionsBootstrapper.create(
                env={}, args=["--pants-config-files=['{}']".format(config1)]
            )
