...
class FSTest(TestBase, SchedulerTestBase, metaclass=ABCMeta):
    ...
    def assert_content(self, filespecs_or_globs, expected_content):
        with self.mk_project_tree() as project_tree:
            ...
            snapshot = self.execute_expecting_one_result(
                scheduler, Snapshot, self.specs(filespecs_or_globs)
            ).value
    ...
    def test_walk_escaping_symlink(self):
        ...
        exc_reg = ".*While expanding link.*{}.*may not traverse outside of the buildroot.*{}.*".format(
            link, dest
        )
    ...
    def test_add_prefix(self):
        input_files_content = InputFilesContent(
            (
                ...
                FileContent(
                    path="subdir/sub.py", content=b'print("from sub")', is_executable=False
                ),
            )
        )
        ...
        with temporary_dir() as temp_dir:
            ...
            stripped_digest = assert_single_element(
                self.scheduler.product_request(
                    ...
                    [
                        DirectoryWithPrefixToStrip(
                            snapshot.directory_digest, "characters/dark_tower"
                        )
                    ],
                )
            )
    ...
    def test_download_missing_file(self):
        with self.isolated_local_store():
            with http_server(StubHandler) as port:
                url = UrlToFetch(
                    "http://localhost:{}/notfound".format(port), self.pantsbuild_digest
                )
