        ...
        af = run_rule(
            ...
            Dir("/dev/null"),
            {
                (Snapshot, PathGlobs): lambda _: Snapshot(
                    Digest("abc", 10), ("/dev/null/BUILD",), ()
                ),
                (FilesContent, Digest): lambda _: FilesContent(
                    [FileContent("/dev/null/BUILD", b"", False)]
                ),
            },
        )
...
class InlinedGraphTest(GraphTestBase):
    ...
    def assert_resolve_failure_type(self, expected_type, mismatch, scheduler):
        ...
        self.assertEqual(
            type(failure),
            expected_type,
            "type was not {}. Instead was {}, {!r}".format(
                expected_type.__name__, type(failure).__name__, failure
            ),
        )
