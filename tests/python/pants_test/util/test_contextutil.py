...
class ContextutilTest(unittest.TestCase):
    ...
    def test_open_zip_returns_realpath_on_badzipfile(self) -> None:
        ...
        with temporary_file() as not_zip:
            with temporary_dir() as tempdir:
                ...
                with self.assertRaisesRegexp(zipfile.BadZipfile, f"{not_zip.name}"), open_zip(
                    file_symlink
                ):
                    pass
    ...
    def _stdio_as_tempfiles(self) -> Iterator[None]:
        ...
        with temporary_file(binary_mode=False) as tmp_stdin, temporary_file(
            binary_mode=False
        ) as tmp_stdout, temporary_file(binary_mode=False) as tmp_stderr:
    ...
    def test_stdio_as(self) -> None:
        self.assertTrue(
            sys.stderr.fileno() > 2, "Expected a pseudofile as stderr, got: {}".format(sys.stderr)
        )
