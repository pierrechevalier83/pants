...
class Bundles:
    ...
    lesser_of_two = Bundle("lesser-of-two", "One must choose the lesser of two eval()s.")
    once_upon_a_time = Bundle(
        "once-upon-a-time", "Once upon a time, in a far away land, there were some pants."
    )
    ten_thousand = Bundle(
        "ten-thousand", "And now you must face my army of ten thousand BUILD files."
    )
    there_was_a_duck = Bundle(
        "there-was-a-duck", "And also, there was a duck, with her trusty companion."
    )
...
class ExcludeTargetRegexpIntegrationTest(PantsRunIntegrationTest):
    ...
    def _bundle_path(self, bundle):
        return os.path.join(
            get_buildroot(),
            "dist",
            "{prefix}.{name}-bundle".format(prefix=Bundles.bundle_dir_prefix, name=bundle),
        )
    ...
    def _handle_bundles(self, names):
        ...
        for path in paths:
            if os.path.exists(path):
                ...
                missing.append(path)
        self.assertFalse(
            missing, "Some bundles weren't generated! {missing}".format(missing=", ".join(missing))
        )
    ...
    def _test_bundle_existences(self, args, bundles, config=None):
            ...
            with self._handle_bundles(names) as (paths, jars):
                for path, jar, expected in zip(paths, jars, outputs):
                    java_run = subprocess.Popen(
                        ["java", "-jar", jar], stdout=subprocess.PIPE, cwd=path
                    )
                    ...
                    self.assertTrue(
                        ...
                        "Expected '{output}' from {jar}, not '{stdout}'.".format(
                            output=expected, jar=jar, stdout=java_out
                        ),
                    )
        ...
        self.assertTrue(
            not lingering,
            "Left with unexpected bundles! {bundles}".format(bundles=", ".join(lingering)),
        )

    def test_single_run(self):
        """Test whether we can run a single target without special flags."""
        self._test_bundle_existences(
            [Bundles.lesser_of_two.full_spec], [Bundles.lesser_of_two],
        )
    ...
    def test_all_run(self):
        ...
        self._test_bundle_existences(
            [Bundles.phrase_path + "::"], Bundles.all_bundles,
        )
    ...
    def test_only_exclude_roots(self):
        ...
        self._test_bundle_existences(
            [
                ...
                "--exclude-target-regexp={}:{}".format(
                    Bundles.phrase_path, Bundles.trusty_companion
                ),
            ],
