...
class DistributionIntegrationTest(PantsRunIntegrationTest):
    ...
    def test_no_jvm_restriction(self):
        with _distribution_locator() as locator:
            ...
            self.assert_success(run)
            self.assertIn(
                "java.home:{}".format(os.path.realpath(distribution.home)), run.stdout_data
            )
    ...
    def _test_jvm_does_not_meet_distribution_requirements(
        ...
        with _distribution_locator(distribution_locator_options) as locator:
            with self.assertRaises(Distribution.Error):
                locator.cached(
                    minimum_version=min_version_arg, maximum_version=max_version_arg, jdk=False
                )
    ...
    def test_min_option_trumps_min_arg(self):
        self._test_jvm_does_not_meet_distribution_requirements(
            min_version_arg=self.BOTTOM, min_version_option=self.TOP
        )

    def test_min_arg_trumps_min_option(self):
        self._test_jvm_does_not_meet_distribution_requirements(
            min_version_arg=self.TOP, min_version_option=self.BOTTOM
        )

    def test_max_option_trumps_max_arg(self):
        self._test_jvm_does_not_meet_distribution_requirements(
            max_version_arg=self.TOP, max_version_option=self.BOTTOM
        )

    def test_max_arg_trumps_max_option(self):
        self._test_jvm_does_not_meet_distribution_requirements(
            max_version_arg=self.BOTTOM, max_version_option=self.TOP
        )
