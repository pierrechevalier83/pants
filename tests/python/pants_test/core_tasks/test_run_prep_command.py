...
class RunPrepCommandTest(TaskTestBase):
    ...
    def test_prep_command_fails(self):
        with self.assertRaises(TaskError):
            a = self.make_target(
                ...
                target_type=PrepCommand,
                prep_executable="mv",
                prep_args=["/non/existent/file/name", "/bogus/destination/place"],
            )
    ...
    def test_valid_target_single_goal(self):
        prep_command = self.make_target(
            "foo", PrepCommand, prep_executable="foo.sh", goals=["binary"]
        )
        self.assertEqual(frozenset({"binary"}), prep_command.goals)

    def test_valid_target_multiple_goals(self):
        prep_command = self.make_target(
            "foo", PrepCommand, prep_executable="foo.sh", goals=["binary", "test"]
        )
        self.assertEqual(frozenset({"binary", "test"}), prep_command.goals)

    def test_invalid_target_no_executable(self):
        with self.assertRaisesRegexp(
            TargetDefinitionException, r"prep_executable must be specified"
        ):
            self.make_target(
                "foo", PrepCommand,
            )
    ...
    def test_invalid_target_unrecognized_goals(self):
        with self.assertRaisesRegexp(
            ...
            r".*Got unrecognized goals baloney, malarkey. " r"Goal must be one of.*",
        ):
            self.make_target(
                "foo", PrepCommand, prep_executable="foo.sh", goals=["baloney", "malarkey", "test"]
            )

    def test_runnable_prep_cmd(self):
        test_prep_cmd = self.make_target("test-prep-cmd", PrepCommand, prep_executable="foo.sh")
        binary_prep_cmd = self.make_target(
            "binary-prep-cmd", PrepCommand, prep_executable="foo.sh", goals=["binary"]
        )
