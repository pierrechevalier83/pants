  ...
  """
    ...
    def _create_graph(self):
        ...
        self.a_library = self.create_python_library(
            "src/python/a", "a", {"a_1.py": self.BAD_IMPORT_ORDER, "a_2.py": self.BAD_IMPORT_ORDER}
        )
        ...
        self.b_library = self.create_python_library(
            "src/python/a/b", "b", {"b.py": self.BAD_IMPORT_ORDER}
        )

        self.c_library = self.create_python_library(
            "src/python/c", "c", {"c.py": self.BAD_IMPORT_ORDER}
        )
    ...
    def _create_task(self, target_roots, options=None, passthru_args=None):
        ...
        context = self.context(
            for_task_types=[isort_prep_task_type],
    ...
    def test_isort_check_only(self):
        isort_task = self._create_task(
            target_roots=[self.a_library], passthru_args=["--check-only"]
        )
            ...
            with open(os.path.join(output_dir, "stdout"), "w+") as stdout:
                with stdio_as(stdout_fd=stdout.fileno(), stderr_fd=stdout.fileno(), stdin_fd=-1):
                    try:
                        ...
                        fail(
                            f"--check-only test for {self.a_library} is supposed to fail, but passed."
                        )

    def assertSortedWithConfigA(self, path):
        with open(path, "r") as f:
            self.assertEqual(
                self.RESULT_A, f.read(), f"{path} should be sorted with CONFIG_A, but is not."
            )

    def assertSortedWithConfigB(self, path):
        with open(path, "r") as f:
            self.assertEqual(
                self.RESULT_B, f.read(), f"{path} should be sorted with CONFIG_B, but is not."
            )

    def assertNotSorted(self, path):
        with open(path, "r") as f:
            self.assertEqual(
                self.BAD_IMPORT_ORDER, f.read(), f"{path} should not be sorted, but is."
            )
    ...
    def test_skip_glob_config(self):
        self.create_file(
            "src/python/a/.isort.cfg", mode="a", contents="skip_glob=src/python/a/*2.py"
        )
        isort_task = self._create_task(target_roots=[self.a_library])
        isort_task.execute()
        self.assertSortedWithConfigA(os.path.join(self.build_root, "src/python/a/a_1.py"))
