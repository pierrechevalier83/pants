...
class PythonReplTest(PythonTaskTestBase):
    ...
    def alias_groups(cls):
        return super().alias_groups().merge(BuildFileAliases(targets={"jvm_target": cls.JvmTarget}))
    ...
    def setUp(self):
        ...
        self.six = self.create_python_requirement_library(
            "3rdparty/python/six", "six", requirements=["six==1.9.0"]
        )
        self.requests = self.create_python_requirement_library(
            "3rdparty/python/requests", "requests", requirements=["requests==2.6.0"]
        )
        ...
        self.binary = self.create_python_binary(
            "src/python/bin", "bin", "lib.go", dependencies=["//src/python/lib"]
        )
    ...
    def new_io(self, stdin_data):
        with temporary_dir() as iodir:
            ...
            with open(stdin, "r") as inp, open(stdout, "w") as out, open(stderr, "w") as err:
                with stdio_as(
                    stdin_fd=inp.fileno(), stdout_fd=out.fileno(), stderr_fd=err.fileno()
                ):
    ...
    def do_test_repl(self, code, expected, targets, options=None):
        ...
        context = self.context(
            for_task_types=[si_task_type, rr_task_type, gs_task_type], target_roots=targets
        )
    ...
    def do_test_library(self, *targets):
        self.do_test_repl(
            code=["from lib.lib import go", "go()"], expected=["gogogo!"], targets=targets
        )
    ...
    def test_requirement(self):
        self.do_test_repl(
            code=["import six", 'print("python 2?:{}".format(six.PY2))'],
    ...
    def test_mixed_python(self):
        self.do_test_repl(
            ...
            expected=["teapot response code is: 418", "gogogo!", "python 2?:True"],
    ...
    def test_non_python_targets(self):
        self.do_test_repl(
            code=["import java.lang.unreachable"], expected=[""], targets=[self.non_python_target]
        )
