    ...
    def test_ctypes_binary_creation(self, toolchain_variant):
        ...
        with temporary_dir() as tmp_dir:
            ...
            compiler_names_to_check = toolchain_variant.match(
                {ToolchainVariant.gnu: ["gcc", "g++"], ToolchainVariant.llvm: ["clang", "clang++"],}
            )
            for compiler_name in compiler_names_to_check:
                self.assertIn(
                    "selected compiler exe name: '{}'".format(compiler_name), pants_run.stdout_data
                )
            ...
            for linker_name in linker_names_to_check:
                self.assertIn(
                    "selected linker exe name: '{}'".format(linker_name), pants_run.stdout_data
                )
            ...
            wheel_dist = re.sub(
                "^{}{}".format(re.escape(tmp_dir), os.path.sep), "", wheel_dist_with_path
            )
