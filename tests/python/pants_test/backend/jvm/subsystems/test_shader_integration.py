...
class ShaderIntegrationTest(PantsRunIntegrationTest):
    ...
    """
        ...
        self.assertEqual(
            0, execute_java(classpath=[path], main="org.pantsbuild.testproject.shading.Main")
        )
        self.assertEqual(
            0,
            execute_java(
                classpath=[path], main="org.pantsbuild.testproject.foo.bar.MyNameIsDifferentNow"
            ),
        )
        ...
        self.assertEqual(
            ...
            json.loads(
                self.bundle_and_run(
                    ...
                    expected_bundle_content=["third.jar"],
                ).strip()
            ),
        )
