...
class ClasspathProductsTest(TestBase):
    ...
    def test_copy(self):
        ...
        self.assertEqual(
            [("default", resolved_jar.pants_path), ("default", self.path("a/path"))],
        ...
        self.assertEqual([("default", self.path("a/path"))], copied.get_for_targets(a_closure))
        ...
        self.assertEqual(
            [("default", self.path("a/path")), ("default", self.path("b/path"))],
            copied.get_for_targets(a_closure),
        )
    ...
    def test_fails_if_jar_paths_outside_buildroot(self):
        ...
        with self.assertRaises(TaskError) as cm:
            classpath_product.add_jars_for_targets(
                [a], "default", [(resolved_example_jar_at("/dev/null"))]
            )
    ...
    def test_jar_provided_by_transitive_target_excluded(self):
        provider = self.make_target(
            "provider", ExportableJvmLibrary, provides=Artifact("com.example", "lib", Repository())
        )
    ...
    def test_jar_missing_pants_path_fails_adding(self):
        ...
        self.assertEqual("Jar: org:name: has no specified path.", str(cm.exception))
    ...
    def test_get_product_target_mappings_for_targets_ignore_excludes(self):
        ...
        example_jar_path = self._example_jar_path()
        resolved_jar = self.add_jar_classpath_element_for_path(
            classpath_product, a, example_jar_path, conf="fred-conf"
        )
        self.add_excludes_for_targets(classpath_product, a)

        classpath_target_tuples = classpath_product.get_product_target_mappings_for_targets(
            [a], respect_excludes=False
        )

        expected_entry = ArtifactClasspathEntry(
            example_jar_path, resolved_jar.coordinate, resolved_jar.cache_path
        )
    ...
    def test_get_classpath_entries_for_targets_dedup(self):
        ...
        self.add_jar_classpath_element_for_path(
            classpath_product, b, example_jar_path, conf="fred-conf"
        )
        classpath_target_tuples = classpath_product.get_classpath_entries_for_targets(
            [a], respect_excludes=False
        )
    ...
    def test_create_canonical_classpath(self):
        ...
        classpath_products.add_for_target(
            a,
            [("default", self._create_file("a.jar")), ("default", self._create_file("resources"))],
        )
        ...
        classpath_products = ClasspathProducts(self.pants_workdir)
        classpath_products.add_for_target(
            a, [("default", self._create_file("a.jar")), ("default", self._create_file("b.jar"))]
        )
        self._test_canonical_classpath_helper(
            classpath_products,
            [a],
            ...
            {
                "a.b.b-classpath.txt": "{}/a.jar:{}/b.jar\n".format(
    ...
    """
        ...
        self.assertEqual(
            expected_canonical_classpath, relativize_paths(canonical_classpath, libs_dir)
        )
    ...
    @staticmethod
    def add_jar_classpath_element_for_path(classpath_product, target, example_jar_path, conf=None):
        resolved_jar = resolved_example_jar_at(example_jar_path)
        classpath_product.add_jars_for_targets(
            targets=[target], conf=conf or "default", resolved_jars=[resolved_jar]
        )
        return resolved_jar
