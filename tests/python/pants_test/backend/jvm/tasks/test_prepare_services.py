...
class PrepareServicesTest(TaskTestBase):
    ...
    def test_find_all_relevant_resources_targets(self):
        ...
        java_library2 = self.make_target(
            "java:target2",
        ...
        task = self.create_task(
            self.context(
                target_roots=[jvm_target, java_library, non_services_target, java_library2]
            )
        )
    ...
    def test_create_invalidation_strategy(self):
        ...
        self.reset_build_graph()
        target = self.make_target(
            "java:target", excludes=[Exclude("com.foo", "fiz")], target_type=JavaLibrary, sources=[]
        )
        ...
        target = self.make_target(
            ...
            target_type=JavaLibrary,
            sources=[],
            services={"com.foo.bars.Baz": ["com.spam.bars.BazImplA", "com.spam.bars.BazImplB"]},
        )
    ...
    def test_prepare_resources_none(self):
        ...
        assert_no_resources_prepared(
            self.make_target("java:target", target_type=JavaLibrary, sources=[])
        )
        assert_no_resources_prepared(
            self.make_target("java:target2", target_type=JavaLibrary, sources=[], services={})
        )
        assert_no_resources_prepared(
            self.make_target("jvm:target", target_type=JvmTarget, services={"ServiceInterface": []})
        )
    ...
    def test_prepare_resources(self):
        ...
        with temporary_dir() as chroot:
            ...
            self.assertEqual(
                sorted(
                    os.path.join("META-INF", "services", svc)
