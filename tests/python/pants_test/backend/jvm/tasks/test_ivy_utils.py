...
class IvyUtilsGenerateIvyTest(IvyUtilsTestBase):
    ...
    def test_get_resolved_jars_for_coordinates(self):
        ...
        resolved_jars = ivy_info.get_resolved_jars_for_coordinates(
            [JarDependency(org="org1", name="name1", rev="0.0.1", classifier="tests")]
        )

        expected = {
            "ivy2cache_path/org1/name1.jar": coord(org="org1", name="name1", classifier="tests"),
    ...
    def test_resolved_jars_with_different_version(self):
        ...
        ivy_info = self.parse_ivy_report(
            "ivy_utils_resources/report_with_resolve_to_other_version.xml"
        )
        ...
        self.maxDiff = None
        self.assertEqual(
            [coord(org="org1", name="name1", classifier="tests", rev="0.0.2")],
    ...
    def test_retrieve_resolved_jars_with_coordinates_differing_on_version_on_flat_fetch_resolve(
        self,
    ):
        ivy_info = self.parse_ivy_report("ivy_utils_resources/report_with_flat_graph.xml")
        ...
        self.assertEqual(
            [coord(org="org2", name="name2", rev="0.0.1")], [r.coordinate for r in result]
        )

    def test_parse_fails_when_same_classifier_different_type(self):
        with self.assertRaises(IvyResolveMappingError):
            self.parse_ivy_report(
                "ivy_utils_resources/report_with_same_classifier_different_type.xml"
            )
        ...
        with temporary_dir() as mock_cache_dir:
            with temporary_dir() as hardlink_dir:
                with temporary_dir() as classpath_dir:
                    ...
                    result_classpath, result_map = IvyUtils.construct_and_load_hardlink_map(
                        hardlink_dir, mock_cache_dir, input_path, output_path
                    )
                    hardlink_foo_path = os.path.join(hardlink_dir, "foo.jar")
                    self.assertEqual([hardlink_foo_path], result_classpath)
                    self.assertEqual({os.path.realpath(foo_path): hardlink_foo_path}, result_map)
                    ...
                    with open(output_path, "r") as outpath:
                        self.assertEqual(
                            hardlink_foo_path + os.pathsep + hardlink_bar_path, outpath.readline()
                        )
                    self.assertFalse(os.path.islink(hardlink_foo_path))
                    self.assertTrue(os.path.exists(hardlink_foo_path))
                    ...
                    IvyUtils.construct_and_load_hardlink_map(
                        hardlink_dir, mock_cache_dir, input_path, output_path
                    )
                    with open(output_path, "r") as outpath:
                        self.assertEqual(
                            hardlink_bar_path + os.pathsep + hardlink_foo_path, outpath.readline()
                        )

    def test_missing_ivy_report(self):
        self.set_options_for_scope(IvySubsystem.options_scope, cache_dir="DOES_NOT_EXIST")

        with self.assertRaises(IvyUtils.IvyResolveReportError):
            IvyUtils.parse_xml_report(
                "default",
                IvyUtils.xml_report_path(
                    "INVALID_CACHE_DIR", "INVALID_REPORT_UNIQUE_NAME", "default"
                ),
            )
        ...
        self.assertEqual(
            {
                IvyModule(
                    ref=IvyModuleRef(
                        org="org1", name="name1", rev="0.0.1", classifier=None, ext=u"jar"
                    ),
                ...
                IvyModule(
                    ref=IvyModuleRef(
                        org="org1", name="name1", rev="0.0.1", classifier="wut", ext=u"jar"
                    ),
    ...
    def test_fetch_ivy_xml_requests_url_for_dependency_containing_url(self):
        with temporary_dir() as temp_dir:
            ...
            IvyUtils.generate_fetch_ivy(
                [JarDependency("org-f", "name-f", "rev-f", url="an-url")],
                ivyxml,
                ("default",),
                "some-name",
            )
    ...
    def test_fetch_requests_classifiers(self):
        with temporary_dir() as temp_dir:
            ivyxml = os.path.join(temp_dir, "ivy.xml")
            IvyUtils.generate_fetch_ivy(
                [JarDependency("org-f", "name-f", "rev-f", classifier="a-classifier")],
                ivyxml,
                ("default",),
                "some-name",
            )
    ...
    def test_fetch_applies_mutable(self):
        with temporary_dir() as temp_dir:
            ivyxml = os.path.join(temp_dir, "ivy.xml")
            IvyUtils.generate_fetch_ivy(
                [JarDependency("org-f", "name-f", "rev-f", mutable=True)],
                ivyxml,
                ("default",),
                "some-name",
            )
    ...
    def test_resolve_ivy_xml_requests_classifiers(self):
        with temporary_dir() as temp_dir:
            ...
            IvyUtils.generate_ivy(
                ...
                jar_dep_manager=namedtuple("stub_jar_dep_manager", ["resolve_version_conflict"])(
                    lambda x: x
                ),
            )
    ...
    def test_ivy_resolve_report_copying_fails_when_report_is_missing(self):
        with temporary_dir() as dir:
            with self.assertRaises(IvyUtils.IvyError):
                IvyUtils._copy_ivy_reports(
                    {"default": os.path.join(dir, "to-file")}, ["default"], dir, "another-hash-name"
                )
...
class IvyFrozenResolutionTest(TestBase):
            ...
            json.dump(
                {
                    "default": {
                        "coord_to_attrs": {},
                        "target_to_coords": {"non-existent-target": []},
                    }
                },
                resolve_file,
            )
    ...
    def test_jar_relative_url(self):
        ...
        def verify_url_attributes(spec, jar, expected_attributes):
            ...
            self.assertEqual(
                list(frozen_resolution.coordinate_to_attributes.values()), expected_attributes
            )
