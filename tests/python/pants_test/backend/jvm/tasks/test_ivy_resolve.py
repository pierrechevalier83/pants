...
class IvyResolveTest(NailgunTaskTestBase):
    ...
    def setUp(self):
        ...
        self.set_options_for_scope(
            "cache.{}".format(self.options_scope), read_from=None, write_to=None
        )
    ...
    def test_resolve_conflicted(self):
        ...
        self.assertEqual(
            OrderedSet(
                [(u"default", artifact_path(u"bogus0")), (u"default", artifact_path(u"bogus1"))]
            ),
            winning_cp,
        )
    ...
    def test_excludes_in_java_lib_excludes_all_from_jar_lib(self):
        ...
        excluding_target = self.make_target(
            "//:b", JavaLibrary, sources=[], excludes=[Exclude("junit", "junit")]
        )
    ...
    def test_second_resolve_reuses_existing_resolution_files(self):
        ...
        with self._temp_workdir():
            ...
            self.assertEqual(
                initial_context.products.get_data("compile_classpath"),
    ...
    def test_fetch_has_same_resolved_jars_as_resolve(self):
        ...
        with self._temp_cache_dir():
            ...
            self._assert_classpath_equal_for_target(
                junit_jar_lib, fetch_classpath, fetch_workdir, resolve_classpath, resolve_workdir
            )
    ...
    def test_loading_valid_resolve(self):
        ...
        self.assertEqual(
            context1.products.get_data("compile_classpath"),
    ...
    def test_fetch_jar_with_url_specified(self):
        ...
        with self._temp_cache_dir(), temporary_file_path() as jarfile:
            jar_with_url = JarDependency(
                "org1", "name1", rev="1234", url="file://{}".format(jarfile)
            )
    ...
    def _temp_cache_dir(self):
        with temporary_dir() as cache_dir:
            self.set_options_for_scope(
                "cache.{}".format(self.options_scope), read_from=[cache_dir], write_to=[cache_dir]
            )
            yield

    def _assert_classpath_equal_for_target(
        self,
        target,
        actual_classpath_product,
        actual_workdir,
        expected_classpath_product,
        expected_workdir,
    ):
        self.assertEqual(
    ...
    def _assertIsFile(self, path):
        self.assertTrue(os.path.isfile(path), "Expected {} to exist as a file".format(path))
...
class IvyResolveFingerprintStrategyTest(TaskTestBase):
    ...
    def test_jvm_target_with_excludes_is_hashed(self):
        ...
        target_with_excludes = self.make_target(
            ":jvm-target", target_type=JvmTarget, excludes=[Exclude("org.some")]
        )
    ...
    def test_jar_library_with_one_jar_is_hashed(self):
        ...
        jar_library = self.make_target(
            ":jar-library", target_type=JarLibrary, jars=[JarDependency("org.some", "name")]
        )
