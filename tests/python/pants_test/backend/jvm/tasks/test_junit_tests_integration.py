...
class JunitTestsIntegrationTest(PantsRunIntegrationTest):
    ...
    def test_junit_test_custom_interpreter(self):
        with self.temporary_workdir() as workdir:
            ...
            self._assert_output_for_class(
                workdir=workdir, classname="org.pantsbuild.example.hello.greet.GreetingTest"
            )
            self._assert_output_for_class(
                workdir=workdir, classname="org.pantsbuild.example.hello.welcome.WelSpec"
            )
    ...
    def test_junit_test(self):
        ...
        with self.temporary_file_content(
            f"{empty_directory_path}/BUILD", empty_target
    ...
    def test_junit_test_early_exit(self):
        pants_run = self.run_pants(
            ["test", "testprojects/src/java/org/pantsbuild/testproject/junit/earlyexit:tests"]
        )
        self.assert_failure(pants_run)
        self.assertIn(
            "java.lang.UnknownError: Abnormal VM exit - test crashed.", pants_run.stdout_data
        )
    ...
    def test_junit_test_target_cwd(self):
        pants_run = self.run_pants(
            ["test", "testprojects/tests/java/org/pantsbuild/testproject/workdirs/onedir"]
        )
        self.assert_success(pants_run)

    def test_junit_test_annotation_processor(self):
        pants_run = self.run_pants(
            ["test", "testprojects/tests/java/org/pantsbuild/testproject/annotation"]
        )
        self.assert_success(pants_run)

    def test_junit_test_256_failures(self):
        pants_run = self.run_pants(
            ["test", "testprojects/tests/java/org/pantsbuild/testproject/fail256"]
        )
        self.assert_failure(pants_run)
        self.assertIn("Failures: 256", pants_run.stdout_data)

    def test_junit_test_duplicate_resources(self):
        pants_run = self.run_pants(["test", "testprojects/maven_layout/junit_resource_collision"])
        self.assert_success(pants_run)
    ...
    def test_junit_test_failure_summary(self):
        with self.temporary_workdir() as workdir:
            ...
            with self.source_clone(failing_tree) as failing:
                ...
                pants_run = self.run_pants_with_workdir(
                    ["test.junit", "--failure-summary", failing_addr], workdir
                )
                self.assert_failure(pants_run)
                expected_groups = []
                expected_groups.append(
                    ["org/pantsbuild/tmp/tests:one", "org.pantsbuild.tmp.tests.OneTest#testSingle"]
                )
                expected_groups.append(
                    [
    ...
    def test_junit_test_no_failure_summary(self):
        with self.temporary_workdir() as workdir:
            ...
            with self.source_clone(failing_tree) as failing:
                failing_addr = os.path.join(failing, "tests", "org", "pantsbuild", "tmp", "tests")
                pants_run = self.run_pants_with_workdir(
                    ["test.junit", "--no-failure-summary", failing_addr], workdir
                )
                self.assert_failure(pants_run)
                output = "\n".join(line.strip() for line in pants_run.stdout_data.split("\n"))
                self.assertNotIn(
    ...
    def test_junit_test_successes_and_failures(self):
        with self.temporary_workdir() as workdir:
            ...
            with self.source_clone(mixed_tree) as mixed:
                ...
                pants_run = self.run_pants_with_workdir(
                    ["test.junit", "--failure-summary", "--no-fail-fast", mixed_addr], workdir
                )
                group = [
