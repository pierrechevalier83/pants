...
class JarCreateTestBase(JarTaskTestBase):
    ...
    def setUp(self):
        super().setUp()
        self.set_options(
            compressed=False, pants_bootstrapdir="~/.cache/pants", max_subprocess_args=100
        )
...
class JarCreateMiscTest(JarCreateTestBase):
    ...
    def test_resources_with_scala_java_files(self):
        for ftype in ("java", "scala"):
            target = self.create_resources(
                os.path.join("project", ftype), "target_%s" % ftype, "hello.%s" % ftype
            )
...
class JarCreateExecuteTest(JarCreateTestBase):
    ...
    def setUp(self):
        ...
        self.java_lib_foo = self.java_library(
            test_path("src/java/com/twitter/foo"), "java_foo", ["java_foo.java"]
        )
    ...
    def test_classfile_jar_contents(self):
        ...
        self.add_to_runtime_classpath(
            context, self.scala_lib, idict("scala_foo.class", "java_foo.class")
        )
        self.add_to_runtime_classpath(context, self.res, idict("r.txt.transformed"))

        self.execute(context)

        self.assert_jar_contents(
            context, "jars", self.jl, ["a.class", "b.class"], ["r.txt.transformed"]
        )
        ...
        self.assert_jar_contents(
            context, "jars", self.scala_lib, ["scala_foo.class", "java_foo.class"]
        )
