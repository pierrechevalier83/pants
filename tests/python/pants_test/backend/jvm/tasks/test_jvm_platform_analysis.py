...
class JvmPlatformValidateTest(JvmPlatformAnalysisTestMixin, TaskTestBase):
    ...
    def assert_depmaps_equal(self, expected, received):
        jvm_deps = {
            target.name: "".join(sorted({t.name for t in deps}))
        ...
        for target, deps in sorted(expected.items()):
            ...
            self.assertEqual(
                ...
                "{}\n  expected {}\n  got {}\n  \n{}".format(
                    ...
                    deps,
                    got,
                    "\n".join("{}: {}".format(key, val) for key, val in sorted(jvm_deps.items())),
                ),
            )
        self.assertEqual(
            len(list(filter(expected.get, expected))), len(list(filter(jvm_deps.get, jvm_deps)))
        )
    ...
    def test_non_jvm_transitivity(self):
        ...
        jvm_deps = self.simple_task(
            self.construct_interesting_graph()
        )._unfiltered_jvm_dependency_map()
...
class JvmPlatformExplainTest(JvmPlatformAnalysisTestMixin, TaskTestBase):
    ...
    def test_broken(self):
        ...
        targets = [one, two]
        expected = (
            "Allowable JVM platform ranges (* = anything):",
            "java:one: <=1.6  (is 1.7)",
    ...
    def test_upgradeable(self):
        ...
        text = "\n".join(
            self.get_lines([one, two, three], colors=False, ranges=False, upgradeable=True)
        )
    ...
    def test_downgradeable(self):
        ...
        text = "\n".join(
            self.get_lines([one, two, nope], colors=False, ranges=False, downgradeable=True)
        )
