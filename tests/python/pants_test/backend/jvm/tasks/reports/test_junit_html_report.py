...
class TestJUnitHtmlReport(TestBase):
    ...
    def test_unicode(self):
        ...
        self.assertIn("org.pantsbuild.PåssingTest.testTwö", testsuites[0].testcases[1].error)
    ...
    def test_no_open_report(self):
        with temporary_dir() as output_dir:
            junit_html_report = JUnitHtmlReport.create(
                xml_dir=self._JUNIT_XML_DIR, open_report=False
            )
            report_file_path = junit_html_report.report(output_dir)
    ...
    def test_merged_no_conflict(self):
        with temporary_dir() as xml_dir:
            ...
            testsuites = list(
                ReportTestSuite.merged(JUnitHtmlReport.create(xml_dir)._parse_xml_files())
            )
        ...
        with self.merge_conflict() as xml_dir:
            ...
            self.assertEqual(
                [
                    ...
                    ReportTestCase(name="test-b", time=0.02, error="java.lang.RuntimeException!"),
                ],
                suite_a.testcases,
            )
