...
class BaseCompileIT(PantsRunIntegrationTest):
  ...
  """
    ...
    def do_test_compile(
        ...
        extra_args=None,
        workdir_outside_of_buildroot=False,
    ):
            ...
            with self.temporary_cachedir() as cachedir:
                for i in range(0, iterations):
                    pants_run = self.run_test_compile(
                        workdir, cachedir, target, clean_all=(i == 0), extra_args=extra_args
                    )
                    if expect_failure:
    ...
    """
        ...
        with self.do_test_compile(
            target, extra_args=(shared_args + failure_args), expect_failure=True
        ):
            pass
