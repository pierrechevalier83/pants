...
class JavaCompileSettingsPartitioningTest(NailgunTaskTestBase):
    ...
    def test_independent_targets(self):
        ...
        partition = self._partition(
            [java6, java7, java8], platforms=self._platforms("1.6", "1.7", "1.8")
        )
        expected = {self._version(java.payload.platform): {java} for java in (java6, java7, java8)}
    ...
    def test_unspecified_default(self):
        ...
        self.assert_partitions_equal(
            {self._version("1.6"): {java, java6}, self._version("1.7"): {java7},}, partition
        )
    ...
    def test_invalid_source_target_combination_by_jvm_platform(self):
        ...
        with self.assertRaises(JvmPlatformSettings.IllegalSourceTargetCombination):
            self._settings_and_targets(
                [java_wrong], platforms={"bad": {"source": "1.7", "target": "1.6"}}
            )
    ...
    def test_compile_setting_inequivalence(self):
        ...
        self.assertNotEqual(
            JvmPlatformSettings("1.4", "1.6", ["-Xfoo:bar"]),
            JvmPlatformSettings("1.6", "1.6", ["-Xfoo:bar"]),
        )
    ...
    def test_java_home_extraction_empty(self):
        init_subsystem(DistributionLocator)
        result = tuple(self._get_zinc_arguments(JvmPlatformSettings("1.7", "1.7", [])))
        self.assertEqual(
