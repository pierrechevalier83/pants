        ...
        with self.temporary_workdir() as workdir:
            with self.temporary_cachedir() as cachedir:
                ...
                pants_run = self.run_test_compile(
                    ...
                    target,
                    extra_args=[
                        '--cache-compile-rsc-write-to=["{}/dummy_artifact_cache_dir"]'.format(
                            cachedir
                        ),
                    ],
                    clean_all=True,
            ...
            with self.temporary_workdir() as workdir:
                with self.temporary_cachedir() as cachedir:
                    pants_run = self.run_test_compile(
                        workdir,
                        cachedir,
                        "testprojects/src/java/org/pantsbuild/testproject/compilation_warnings:{}".format(
                            target
                        ),
    ...
    def test_hermetic_incremental_compile(self):
        ...
        with self.temporary_workdir(cleanup=False) as tmp_build_root:
            ...
            with temporary_dir() as cache_dir, temporary_dir(
                root_dir=tmp_build_root, suffix=".pants.d"
            ) as workdir:
                config = {
