...
class JavaCompileIntegrationTest(BaseCompileIT):
    ...
    def test_nocache(self):
        with temporary_dir() as cache_dir:
            ...
            bad_artifact_dir = os.path.join(
                zinc_task_dir, "testprojects.src.java.org.pantsbuild.testproject.nocache.nocache"
            )
            good_artifact_dir = os.path.join(
                zinc_task_dir, "testprojects.src.java.org.pantsbuild.testproject.nocache.cache_me"
            )
    ...
    def create_platform_args(self, version):
        return [
            (
                """--jvm-platform-platforms={{'default': {{'target': '{version}'}}}}""".format(
                    version=version
                )
            ),
    ...
    def test_java_compile_produces_different_artifact_depending_on_java_version(self):
        ...
        with temporary_dir() as cache_dir:
            ...
            java_7_artifact_dir = self.get_cache_subdir(
                cache_dir, other_dirs=[java_6_artifact_dir],
            )
        ...
        with temporary_dir() as cache_dir:
            ...
            with temporary_dir() as extract_dir:
                ...
                self.assertEqual(
                    1,
                    len(reports),
                    "Expected exactly one {} file; got: {}".format(report_file_name, all_files),
                )
    ...
    def test_java_compile_with_changes_in_resources_dependencies(self):
        with self.source_clone(
            "testprojects/src/java/org/pantsbuild/testproject/resdependency"
        ) as resdependency:
            with self.temporary_workdir() as workdir:
                with self.temporary_cachedir() as cachedir:
                    ...
                    with open(
                        os.path.join(resdependency, "resources/resource.xml"), "w"
                    ) as xml_resource:
                    ...
                    self.assertTrue(
                        "Compiling" not in second_run.stdout_data,
    ...
    def test_java_compile_with_different_resolved_jars_produce_different_artifacts(self):
        ...
        with self.temporary_workdir() as workdir, temporary_dir() as cache_dir:
            path_prefix = (
                "testprojects/src/java/org/pantsbuild/testproject/jarversionincompatibility"
            )
            ...
            guava_15_artifact_dir = os.path.join(
                guava_15_base_dir, "{}.jarversionincompatibility".format(dotted_path),
            )
            ...
            self.assert_success(
                self.run_pants_with_workdir(
                    ["compile", (u"{}:alongside-16".format(path_prefix)),], workdir, config
                )
            )
            ...
            guava_16_artifact_dir = os.path.join(
                guava_16_base_dir, "{}.jarversionincompatibility".format(dotted_path),
            )
