...
class ScalastyleTest(NailgunTaskTestBase):
    ...
    def setUp(self):
        ...
        init_subsystem(ScalaPlatform, {ScalaPlatform.options_scope: {"version": "2.11"}})
    ...
    def test_initialize_config_config_setting_exist_but_invalid(self):
        with self.assertRaises(Scalastyle.MissingConfig):
            self._create_scalastyle_task(
                scalastyle_config="file_does_not_exist.xml"
            ).validate_scalastyle_config()
    ...
    def custom_scala_platform_setup(self):
        ...
        init_subsystem(ScalaPlatform, {ScalaPlatform.options_scope: {"version": "custom"}})
    ...
    def test_get_non_synthetic_scala_targets(self):
        ...
        scala_target_with_java_source = self.make_target(
            "a/scala_java:sj", ScalaLibrary, sources=["Source.java"]
        )
        ...
        synthetic_scala_target = self.make_target(
            "a/synthetic_scala:ss",
            ScalaLibrary,
            sources=["SourceGenerated.scala"],
            derived_from=scala_target,
        )

        result_targets = Scalastyle.get_non_synthetic_scala_targets(
    ...
    def test_get_non_excluded_scala_sources(self):
        ...
        scala_target_1 = self.make_target(
            "a/scala_1:s1", ScalaLibrary, sources=["Source1.java", "Source1.scala"]
        )
        ...
        result_sources = task.get_non_excluded_scala_sources(
            task.create_file_excluder(), task.get_non_synthetic_scala_targets(context.targets())
        )
    ...
    def test_end_to_end_pass(self):
        ...
        context = self._create_context(
            scalastyle_config=self._create_scalastyle_config_file(), target_roots=[scala_target]
        )
