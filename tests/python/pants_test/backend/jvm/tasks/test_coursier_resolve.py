...
class CoursierResolveTest(NailgunTaskTestBase):
    ...
    def test_resolve_with_remote_url(self):
        dep_with_url = JarDependency(
            "a", "b", "c", url="http://central.maven.org/maven2/junit/junit/4.12/junit-4.12.jar"
        )
        ...
        self.assertTrue(
            any(self._cache_dir_regex("relative").search(path) for path in paths), str(paths)
        )
    ...
    def test_resolve_with_local_url(self):
        with temporary_file_path(suffix=".jar") as url:
            ...
            paths = [tup[1] for tup in compile_classpath.get_for_target(dep_with_url_lib)]
            self.assertTrue(
                any(self._cache_dir_regex("absolute").search(path) for path in paths), str(paths)
            )
    ...
    def test_resolve_specific_with_sources_javadocs(self):
        ...
        with self._temp_workdir() as workdir:
            ...
            compile_classpath = context.products.get_data(
                "compile_classpath", init_func=ClasspathProducts.init_func(workdir)
            )
            task.resolve(
                [jar_lib, scala_lib], compile_classpath, sources=True, javadoc=True, executor=None
            )

            # Both javadoc and sources jars are added to the classpath product
            self.assertEqual(
                ["default", "src_doc", "src_doc"],
    ...
    def test_resolve_multiple_artifacts(self):
        ...
        classifier = JarDependency(
            "org.apache.commons", "commons-compress", rev="1.4.1", classifier="tests"
        )

        self.set_options_for_scope(
            "coursier", fetch_options=["-A", "jar,bundle,test-jar,maven-plugin,src,doc"]
        )
