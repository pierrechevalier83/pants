...
class JvmAppTest(TestBase):
    def test_simple(self):
        ...
        self.assertEqual(
            [":foo-binary"], list(app_target.compute_dependency_specs(payload=app_target.payload))
        )

    def test_jvmapp_bundle_payload_fields(self):
        app_target = self.make_target(
            ":foo_payload", JvmApp, basename="foo-payload-app", archive="zip"
        )
    ...
    def test_bad_basename(self):
        with self.assertRaisesRegex(
            TargetDefinitionException, r"Invalid target JvmApp.* basename must not equal name."
        ):
    ...
    def test_too_many_binaries_mixed(self):
        ...
        app = self.create_app(
            "src/java/org/archimedes/buoyancy", binary=":bin", dependencies=[bin2]
        )
        with self.assertRaisesRegex(
            TargetDefinitionException,
            r"Invalid target JvmApp.*src/java/org/archimedes/buoyancy:app\).*"
...
class BundleTest(TestBase):
    ...
    def test_bundle_filemap_dest_relative(self):
        ...
        app = self.make_target(
            ...
            JvmApp,
            dependencies=[unused],
            bundles=[_bundle(spec_path)(relative_to="gold", fileset="gold/config/five.xml")],
        )
    ...
    def test_bundle_add(self):
        ...
        bundle = _bundle(spec_path)(
            relative_to="config", fileset=["config/stone/dense.xml", "config/metal/dense.xml"]
        )
    ...
    """
        ...
        app1 = self.make_target(
            Address(spec_path, "app1").spec,
            JvmApp,
            dependencies=[unused],
            bundles=[_bundle(spec_path)(relative_to="config", fileset="config/stone/dense.xml")],
        )
    ...
    def test_rel_path_overrides_context_rel_path_for_explicit_path(self):
        ...
        app = self.make_target(
            ...
            JvmApp,
            dependencies=[unused],
            bundles=[_bundle(spec_path)(rel_path="config", fileset=["a/b"])],
        )
        self.assertEqual(
            {os.path.join(self.build_root, "config/a/b"): "a/b"}, app.bundles[0].filemap
        )
