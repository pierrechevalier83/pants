...
class MinimalCoverTest(BaseMinimalCovertTest):
    ...
    def test_roots(self):
        self.assert_console_output(
            "common/a:a", targets=[self.target("common/a")], extra_targets=[self.target("common/b")]
        )

    def test_nodups(self):
        targets = [self.target("common/a")] * 2
        self.assertEqual(2, len(targets))
        self.assert_console_output("common/a:a", targets=targets)
    ...
    def test_intersection(self):
        self.assert_console_output(
            "overlaps:one",
            "overlaps:two",
            targets=[self.target("overlaps:one"), self.target("overlaps:two")],
        )
