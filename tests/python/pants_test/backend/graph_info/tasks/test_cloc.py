...
class ClocTest(ConsoleTaskTestBase):
    ...
    def test_counts(self):
        ...
        self.create_file(
            "src/java/foo/Bar.java", "// We do not expect this file to appear in counts."
        )
        dep_py_tgt = self.make_target("src/py/dep", PythonLibrary, sources=["dep.py"])
        py_tgt = self.make_target(
            "src/py/foo", PythonLibrary, dependencies=[dep_py_tgt], sources=["foo.py", "bar.py"],
        )
        ...
        res = self.execute_console_task(
            targets=[py_tgt, java_tgt], options={"transitive": True}, scheduler=self.scheduler,
        )
        assert_counts(res, "Python", files=3, blank=2, comment=3, code=3)
        assert_counts(res, "Java", files=1, blank=0, comment=1, code=1)

        res = self.execute_console_task(
            targets=[py_tgt, java_tgt], options={"transitive": False}, scheduler=self.scheduler,
        )
    ...
    def test_ignored(self):
        ...
        res = self.execute_console_task(
            targets=[py_tgt], options={"ignored": True}, scheduler=self.scheduler,
        )
        self.assertEqual(
            ["Ignored the following files:", "src/py/foo/empty.py: zero sized file"],
