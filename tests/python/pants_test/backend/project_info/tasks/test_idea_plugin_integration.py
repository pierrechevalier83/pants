...
class IdeaPluginIntegrationTest(PantsRunIntegrationTest):
    def _do_check(
        ...
        self.assertTrue(
            {p.getAttribute("name") for p in actual_properties}.issubset(valid_property_names)
        )
        ...
        abs_expected_target_specs = [
            os.path.join(get_buildroot(), relative_spec) for relative_spec in expected_targets
        ]
        ...
        incremental_import_props = [
            p for p in actual_properties if p.getAttribute("name") == "incremental_import"
        ]
    ...
    """
        ...
        with self.temporary_workdir() as workdir:
            with temporary_file(root_dir=workdir, cleanup=True) as output_file:
                ...
                self._do_check(
                    project_dir, project_path, target_specs, incremental_import=incremental_import
                )
