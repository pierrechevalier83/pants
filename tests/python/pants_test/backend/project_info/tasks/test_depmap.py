...
class DepmapTest(BaseDepmapTest):
                ...
                dedent(
                    ...
                    """.format(
                        ...
                        extra=(
                            ""
                            if not kwargs
                            else ", ".join("{0}={1}".format(k, v) for k, v in kwargs.items())
                        ),
                    )
                ),
            )
    ...
    def test_empty(self):
        self.assert_console_output_ordered("internal-common.a.a", targets=[self.target("common/a")])

        self.assert_console_output_ordered(
            "internal-common.b.b", targets=[self.target("common/b")],
        )

    def test_java_library(self):
        self.assert_console_output_ordered("internal-common.c.c", targets=[self.target("common/c")])

    def test_python_library(self):
        self.assert_console_output_ordered("internal-common.d.d", targets=[self.target("common/d")])

    def test_python_binary(self):
        self.assert_console_output_ordered("internal-common.e.e", targets=[self.target("common/e")])

    def test_jvm_binary1(self):
        self.assert_console_output_ordered("internal-common.f.f", targets=[self.target("common/f")])

    def test_jvm_binary2(self):
        self.assert_console_output_ordered(
            "internal-common.g.g", "  internal-common.f.f", targets=[self.target("common/g")]
        )

    def test_jvm_app1(self):
        self.assert_console_output_ordered(
            "internal-common.h.h", "  internal-common.f.f", targets=[self.target("common/h")]
        )
