...
class ExportIntegrationTest(ResolveJarsTestMixin, PantsRunIntegrationTest):
    ...
    def run_export(
        ...
        self.assertTrue(
            ...
            msg="Could not find export output file in {out_file}".format(out_file=export_out_file),
        )
    ...
    def evaluate_subtask(self, targets, workdir, load_extra_confs, extra_args, expected_jars):
        ...
        for jar in expected_jars:
            ...
            for path in json_data["libraries"][jar].values():
                self.assertTrue(
                    os.path.exists(path), "Expected jar at {} to actually exist.".format(path)
                )
    ...
    def test_export_jar_path_with_excludes_soft(self):
        with self.temporary_workdir() as workdir:
            ...
            json_data = self.run_export(
                test_target, workdir, load_libs=True, extra_args=["--export-soft-excludes"]
            )
            ...
            self.assertEqual(
                {
                    ...
                    "platforms": {
                        "java7": {"source_level": "1.7", "args": ["-X123"], "target_level": "1.7"},
                        "java8": {"source_level": "1.8", "args": ["-X456"], "target_level": "1.8"},
                    },
                },
