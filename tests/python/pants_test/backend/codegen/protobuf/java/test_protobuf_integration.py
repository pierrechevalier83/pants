...
class ProtobufIntegrationTest(PantsRunIntegrationTest):
    ...
    def test_bundle_protobuf_normal(self):
        with self.pants_results(
            ...
            out_path = os.path.join(
                get_buildroot(),
                "dist",
                ("examples.src.java.org.pantsbuild.example.protobuf.distance" ".distance-bundle"),
            )
            java_run = subprocess.Popen(
                [
                    "java",
                    "-cp",
                    "protobuf-example.jar",
    ...
    def test_bundle_protobuf_imports(self):
        with self.pants_results(
            ...
            out_path = os.path.join(
                get_buildroot(),
                "dist",
                ("examples.src.java.org.pantsbuild.example.protobuf.imports" ".imports-bundle"),
            )
            java_run = subprocess.Popen(
                [
                    "java",
                    "-cp",
                    "protobuf-imports-example.jar",
    ...
    def test_bundle_protobuf_unpacked_jars(self):
        with self.pants_results(
            ...
            self.assertEqual(
                pants_run.returncode,
                PANTS_SUCCEEDED_EXIT_CODE,
                "goal bundle run expected success, got {0}\n"
                "got stderr:\n{1}\n"
                "got stdout:\n{2}\n".format(
                    pants_run.returncode, pants_run.stderr_data, pants_run.stdout_data
                ),
            )
            out_path = os.path.join(
                get_buildroot(),
            ...
            args = [
                "java",
                "-cp",
                "protobuf-unpacked-jars-example.jar",
    ...
    def test_source_ordering(self):
        ...
        all_blocks = list(
            find_protoc_blocks([l.strip() for l in pants_run.stdout_data.split("\n")])
        )
        block_text = "\n\n".join(
            "[block {0}]\n{1}".format(index, "\n".join(block))
        ...
        self.assertEqual(
            ...
            "\n{out}\n\nBLOCKS:\n{blocks}".format(
                expected=3, count=len(all_blocks), out=pants_run.stderr_data, blocks=block_text
            ),
        )
