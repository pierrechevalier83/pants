...
class JaxbGenJavaTest(NailgunTaskTestBase):
    ...
    def test_correct_package(self):
        ...
        self.assertEqual(
            fix("org/pantsbuild/example/foo"),
    ...
    def test_guess_package(self):
        ...
        for prefix in supported_prefixes:
            self.assertEqual(
                guess(".pants.d/foo.bar/{0}/pantsbuild/potato/Potato.java".format(prefix)),
                "{0}.pantsbuild.potato".format(prefix),
                "Failed for prefix {0}: {1}.".format(prefix, guess_history[-1]),
            )
            self.assertEqual(
                guess("{0}/pantsbuild/potato/Potato.java".format(prefix)),
                "{0}.pantsbuild.potato".format(prefix),
                "Failed for prefix {0}: {1}.".format(prefix, guess_history[-1]),
            )
            self.assertEqual(
                guess(
                    "/User/foo/bar/.pants.d/gen/jaxb/foo/bar/"
        ...
        self.assertEqual(
            guess("pantsbuild/potato/Potato.java"),
            "pantsbuild.potato",
    ...
    def test_simple(self):
        ...
        self.assertEqual(
            sorted(["ObjectFactory.java", "Vegetable.java"]),
