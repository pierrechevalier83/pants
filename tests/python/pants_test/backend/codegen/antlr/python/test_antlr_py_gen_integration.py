...
class AntlrPyGenIntegrationTest(PantsRunIntegrationTest):
    ...
    def test_antlr_py_gen_integration(self):
        result = self.run_pants(
            ["run", "testprojects/src/python/antlr:eval-bin", '--run-py-args="123 * 321"']
        )
    ...
    def test_python_invalid_antlr_grammar_fails(self):
        result = self.run_pants(["gen", "testprojects/src/antlr/python/test:antlr_failure"])
        self.assertNotEqual(0, result.returncode)
        self.assertIn(
            "grammar file testprojects/src/antlr/python/test/bogus.g has no rules",
            result.stdout_data,
        )
