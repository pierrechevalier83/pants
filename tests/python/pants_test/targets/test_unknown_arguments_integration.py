...
class TestUnknownArgumentsIntegration(PantsRunIntegrationTest):
    ...
    def test_future_params(self):
        ...
        with self.temp_target_spec(**{param: "value"}) as spec:
            run = self.run_pants(
                [
                    '--target-arguments-ignored={{"java_library": ["{}"]}}'.format(param),
                    "-ldebug",
                    "clean-all",
                    spec,
                ]
            ...
            self.assertIn(
                "{} ignoring the unimplemented arguments: {}".format(spec, param), run.stderr_data
            )
    ...
    def test_unknown_params(self):
        ...
        with self.temp_target_spec(**{future: "value", unknown: "other value"}) as spec:
            run = self.run_pants(
                [
                    '--target-arguments-ignored={{"java_library": ["{}"]}}'.format(future),
                    "clean-all",
                    spec,
                ]
            )
