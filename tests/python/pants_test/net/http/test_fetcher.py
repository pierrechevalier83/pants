...
class FetcherTest(unittest.TestCase):
    ...
    def expect_get(self, url, chunk_size_bytes, timeout_secs, chunks=None, listener=True):
        ...
        self.requests.get.expect_called_once_with(
            url, allow_redirects=True, stream=True, timeout=timeout_secs
        )
        self.response.iter_content.expect_called_once_with(chunk_size=chunk_size_bytes)

    def test_get(self):
        with self.expect_get("http://bar", chunk_size_bytes=1024, timeout_secs=60) as (
            chunks,
            expected_listener_calls,
        ):

            self.fetcher.fetch("http://bar", self.listener, chunk_size_bytes=1024, timeout_secs=60)
    ...
    def test_checksum_listener(self):
        ...
        with self.expect_get("http://baz", chunk_size_bytes=1, timeout_secs=37) as (
            chunks,
            expected_listener_calls,
        ):
    ...
    def test_download_listener(self):
        with self.expect_get("http://foo", chunk_size_bytes=1048576, timeout_secs=3600) as (
            chunks,
            expected_listener_calls,
        ):
    ...
    def test_size_mismatch(self):
        ...
        with self.assertRaises(self.fetcher.Error):
            self.fetcher.fetch("http://foo", self.listener, chunk_size_bytes=1024, timeout_secs=60)

        self.requests.get.assert_called_once_with(
            "http://foo", allow_redirects=True, stream=True, timeout=60
        )
        self.response.iter_content.assert_called_once_with(chunk_size=1024)
        self.assert_listener_calls(
            [self.status_call(200, content_length=11)], chunks, expect_finished=False
        )
        self.response.close.assert_called_once_with()
    ...
    def expect_download(self, path_or_fd=None):
        with self.expect_get("http://1", chunk_size_bytes=13, timeout_secs=13, listener=False) as (
            chunks,
            expected_listener_calls,
        ):

            path = self.fetcher.download(
                "http://1", path_or_fd=path_or_fd, chunk_size_bytes=13, timeout_secs=13
            )
    ...
    def test_progress_listener(self, timer):
        ...
        with self.expect_get(
            "http://baz", chunk_size_bytes=1, timeout_secs=37, chunks=[[1]] * 1024
        ) as (chunks, expected_listener_calls):
