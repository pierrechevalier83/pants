...
class BuildFileParserBasicsTest(TestWithBuildFileParser):
    ...
    def alias_groups(cls):
        return BuildFileAliases(targets={"jvm_binary": ErrorTarget, "java_library": ErrorTarget})
    ...
    def test_name_injection(self):
        ...
        self.assert_parser_error(
            build_file, "Targets in root-level BUILD files must be named explicitly"
        )
    ...
    def test_addressable_exceptions(self):
      ...
      """
        ...
        self.assert_parser_error(
            build_file_d, "dependencies passed to Target constructors must be strings"
        )
...
class BuildFileParserTargetTest(TestWithBuildFileParser):
    ...
    def test_sibling_build_files(self):
      ...
      """
        ...
        self.assertEqual(
            {bar_build_file.relpath, base_build_file.relpath, foo_build_file.relpath},
            {address.rel_path for address in addresses},
        )
        self.assertEqual({"//:base", "//:foo", "//:bat"}, {address.spec for address in addresses})
...
class BuildFileParserExposedContextAwareObjectFactoryTest(TestWithBuildFileParser):
    ...
    class JarLibrary(Target):
        def __init__(self, jars=None, **kwargs):
            super(BuildFileParserExposedContextAwareObjectFactoryTest.JarLibrary, self).__init__(
                **kwargs
            )
            self.jars = jars or []
    ...
    class JvmLibrary(Target):
        def __init__(self, provides=None, **kwargs):
            super(BuildFileParserExposedContextAwareObjectFactoryTest.JvmLibrary, self).__init__(
                **kwargs
            )
    ...
    def create_java_libraries(cls, parse_context):
        def real_create_java_libraries(
            base_name, org="com.twitter", provides_java_name=None, provides_scala_name=None
        ):
            def provides_artifact(provides_name):
                ...
                return parse_context.create_object(
                    "artifact", org=org, name=provides_name, repo=jvm_repo
                )

            parse_context.create_object(
                "java_library",
                name=f"{base_name}-java",
                provides=provides_artifact(provides_java_name),
            )

            parse_context.create_object(
                "scala_library",
                name=f"{base_name}-scala",
    ...
    def test_context_aware_object_factories(self):
        ...
        self.assertEqual(
            {"does_not_exist", "create-java-libraries-scala", "create-java-libraries-java"},
