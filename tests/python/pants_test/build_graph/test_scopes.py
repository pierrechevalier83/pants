...
class ScopesTest(TestBase):
        ...
        for i, scope in enumerate(equivalent_defaults):
            ...
            self.assertEqual(
                ...
                "Expected scope {i}. {received} == {expected}".format(
                    i=i, received=received, expected=expected,
                ),
            )
    ...
    def test_scope_exclude_include_precedence(self):
        self.assertTrue(
            Scopes.RUNTIME.in_scope(include_scopes=Scopes.RUNTIME, exclude_scopes=Scopes.COMPILE)
        )
        self.assertFalse(
            Scopes.RUNTIME.in_scope(include_scopes=Scopes.RUNTIME, exclude_scopes=Scopes.RUNTIME)
        )
...
class ScopedClosureTest(TestBase):
    def assert_closure(
        ...
        self.assert_closure_dfs(
            expected_targets, roots, include_scopes, exclude_scopes, respect_intransitive
        )
        self.assert_closure_bfs(
            expected_targets, roots, include_scopes, exclude_scopes, respect_intransitive
        )
        self.assert_closure_dfs(
            expected_targets,
