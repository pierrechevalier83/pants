...
class BuildGraphTest(TestBase):
        ...
        for src, targets in graph_dict.items():
            ...
            self.add_to_build_file(
                ...
                """target(name='{}', dependencies=[{}])\n""".format(
                    src_name, "'{}'".format("','".join(targets)) if targets else ""
                ),
            )
    ...
    def test_walk_graph(self):
        ...
        def assertDependencyWalk(target, results, postorder=False):
            ...
            self.build_graph.walk_transitive_dependency_graph(
                [target.address], lambda x: targets.append(x), postorder=postorder
            )
            self.assertEqual(results, targets)

        def assertDependeeWalk(target, results, postorder=False):
            targets = []
            self.build_graph.walk_transitive_dependee_graph(
                [target.address], lambda x: targets.append(x), postorder=postorder
            )
            self.assertEqual(results, targets)
    ...
    def test_dependency_walk_prelude_epilogue(self):
        ...
        assertDependencyWalkPreludeEpilogue(
            b, ["b-pre", "a-pre", "a-epi", "a", "b-epi", "b"], postorder=True
        )
    ...
    def test_dependee_walk_prelude_epilogue(self):
        ...
        assertDependeeWalkPreludeEpilogue(
            a, ["a-pre", "b-pre", "b-epi", "b", "a-epi", "a"], postorder=True
        )
    ...
    def test_transitive_subgraph_of_addresses_bfs(self):
        ...
        self.assertEqual(
            [
                t.address.target_name
                for t in self.build_graph.transitive_subgraph_of_addresses_bfs([root])
            ],
            [chr(x) for x in range(ord("a"), ord("o") + 1)],
        )
    ...
    def test_invalid_address(self):
        ...
        self.add_to_build_file(
            "BUILD", 'target(name="a", ' '  dependencies=["non-existent-path:b"],' ")"
        )
    ...
    def test_invalid_address_two_hops(self):
        self.add_to_build_file("BUILD", 'target(name="a", ' '  dependencies=["goodpath:b"],' ")")
        self.add_to_build_file(
            "goodpath/BUILD", 'target(name="b", ' '  dependencies=["non-existent-path:c"],' ")"
        )
        with self.assertRaisesRegexp(
    ...
    def test_synthetic_address(self):
        ...
        self.build_graph.inject_synthetic_target(
            ...
            JarLibrary,
            jars=[JarDependency(org="org.scala-lang", name="scala-library", rev="2.11.5")],
        )
        ...
        def check_funcs(expected, roots, dep_predicate_factory=None, **kwargs):
            ...
            for func in subgraph_funcs:
                ...
                if any(ct > 1 for ct in seen_targets.values()):
                    self.fail(
                        "func {} visited {} more than once.".format(
                            func, ", ".join(t.address for t, ct in seen_targets.items() if ct > 1)
                        )
                    )
