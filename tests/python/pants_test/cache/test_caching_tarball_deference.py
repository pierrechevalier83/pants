...
class LocalCachingTarballDereferenceTest(TaskTestBase):
    ...
    def _prepare_task(self, deference, regular_file, regular_file_in_results_dir):
        ...
        self.add_to_build_file(
            "", 'dummy_cache_library(name = "t", source = "{}")'.format(self._filename)
        )
        ...
        for vt in all_vts:
            ...
            with temporary_dir() as tmpdir:
                ...
                self.assertIsNotNone(
                    file_path,
                    "Cannot find file {} in artifact {}".format(SYMLINK_NAME, artifact_address),
                )
                self.assertFalse(
                    os.path.islink(file_path),
                    "{} in artifact {} should not be a symlink but it is.".format(
    ...
    def test_cache_no_dereference_no_file(self):
        ...
        for vt in all_vts:
            ...
            with temporary_dir() as tmpdir:
                ...
                self.assertTrue(
                    os.path.islink(file_path),
                    "{} in artifact {} should be a symlink but it is not.".format(
                        SYMLINK_NAME, artifact_address
                    ),
                )
