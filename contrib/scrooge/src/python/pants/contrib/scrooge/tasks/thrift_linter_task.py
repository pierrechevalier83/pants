...
class ThriftLinterTask(LintTaskMixin, NailgunTask):
    ...
    def execute(self):
        ...
        subsystem_worker_count_configured = not ScroogeLinter.global_instance().options.is_default(
            "worker_count"
        )
        ...
        with self.invalidated(thrift_targets) as invalidation_check:
            ...
            with self.context.new_workunit("parallel-thrift-linter") as workunit:
                worker_pool = WorkerPool(
                    workunit.parent, self.context.run_tracker, worker_count, workunit.name
                )
