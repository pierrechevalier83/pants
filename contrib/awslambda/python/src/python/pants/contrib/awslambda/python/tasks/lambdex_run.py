            ...
            for vt in invalidation_check.all_vts:
                ...
                if not vt.valid:
                    self.context.log.debug(
                        "Existing lambda for {} is invalid, rebuilding".format(vt.target)
                    )
                ...
                self.context.log.debug(
                    "created {}".format(os.path.relpath(lambda_path, get_buildroot()))
                )

                # Put a copy in distdir.
                lambda_copy = os.path.join(
                    self.get_options().pants_distdir, os.path.basename(lambda_path)
                )
                safe_mkdir_for(lambda_copy)
                atomic_copy(lambda_path, lambda_copy)
                self.context.log.info(
                    "created lambda {}".format(os.path.relpath(lambda_copy, get_buildroot()))
                )
    ...
    def _create_lambda(self, target, lambda_path):
        ...
        with temporary_dir() as tmpdir:
            ...
            workunit_factory = functools.partial(
                self.context.new_workunit, name="run-lambdex", labels=[WorkUnitLabel.TOOL]
            )
            cmdline, exit_code = lambdex.run(
                workunit_factory, ["build", "-e", target.handler, tmp_lambda_path]
            )
            if exit_code != 0:
                raise TaskError(
                    "{} ... exited non-zero ({}).".format(cmdline, exit_code), exit_code=exit_code
                )
