...
class MypyTask(LintTaskMixin, ResolveRequirementsTaskBase):
  ...
  """
    ...
    def _get_mypy_pex(self, py3_interpreter: PythonInterpreter, *extra_pexes: PEX) -> PEX:
        ...
        mypy_version = (
            options.mypy_version if options.is_flagged("mypy_version") else options.version
        )
    ...
    def execute(self):
        ...
        with temporary_file_path() as sources_list_path:
            ...
            if self.get_options().config_file:
                cmd.append(
                    f"--config-file={os.path.join(get_buildroot(), self.get_options().config_file)}"
                )
