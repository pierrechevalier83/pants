...
class AvroJavaGenTest(NailgunTaskTestBase):
    ...
    def alias_groups(cls):
        return (
            super()
            .alias_groups()
            .merge(BuildFileAliases(targets={"java_avro_library": JavaAvroLibrary}))
        )
    ...
    def test_avro_java_gen(self):
        ...
        self.assertEqual(
            task._test_cmd_log[0][:-1], ["compile", "schema", "avro-build/src/avro/schema.avsc"]
        )
        ...
        self.assertEqual(
            task._test_cmd_log[1][:-1], ["compile", "protocol", generated_protocol_json_file]
        )
