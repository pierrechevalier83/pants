...
class ErrorProne(NailgunTask):
    ...
    def register_options(cls, register):
        ...
        cls.register_jvm_tool(
            ...
            "errorprone",
            classpath=[
                JarDependency(org="com.google.errorprone", name="error_prone_core", rev="2.3.1"),
            ],
            main=cls._ERRORPRONE_MAIN,
            custom_rules=[Shader.exclude_package("com.google.errorprone", recursive=True)],
        )
        ...
        cls.register_jvm_tool(
            ...
            "errorprone-javac",
            classpath=[
                JarDependency(org="com.google.errorprone", name="javac", rev="9+181-r4173-1"),
            ],
        )
    ...
    def _is_errorprone_target(self, target):
        if not target.has_sources(self._JAVA_SOURCE_EXTENSION):
            self.context.log.debug(
                "Skipping [{}] because it has no {} sources".format(
        ...
        if target.is_synthetic:
            self.context.log.debug(
                "Skipping [{}] because it is a synthetic target".format(target.address.spec)
            )
            return False
    ...
    def errorprone(self, target):
        ...
        runtime_classpath = [
            jar for conf, jar in runtime_classpaths.get_for_targets(target.closure(bfs=True))
        ]
        ...
        if min_jdk_version.components[0] == 1:
            max_jdk_version = Revision(
                min_jdk_version.components[0], min_jdk_version.components[1], "9999"
            )
        else:
            max_jdk_version = Revision(min_jdk_version.components[0], "9999")
        self.set_distribution(
            minimum_version=min_jdk_version, maximum_version=max_jdk_version, jdk=True
        )
        ...
        if self.dist.version < Revision.lenient("9"):
            ...
            jvm_options.extend(
                ["-Xbootclasspath/p:{}".format(self.tool_classpath("errorprone-javac")[0])]
            )
        ...
        with argfile.safe_args(
            self.calculate_sources(target), self.get_options()
        ) as batched_sources:
            args.extend(batched_sources)
            result = self.runjava(
                classpath=self.tool_classpath("errorprone"),
