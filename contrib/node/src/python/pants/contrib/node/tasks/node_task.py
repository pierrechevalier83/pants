...
class NodeTask(Task):
    ...
    def add_package(
        ...
        command = package_manager.add_package(
            package, type_option=type_option, version_option=version_option, node_paths=node_paths,
        )
        return self._execute_command(
    ...
    def run_script(
        ...
        command = package_manager.run_script(
            script_name, script_args=script_args, node_paths=node_paths,
        )
        return self._execute_command(
            command, workunit_name=workunit_name, workunit_labels=workunit_labels
        )
    ...
    """
        ...
        with self.context.new_workunit(
            name=workunit_name, labels=workunit_labels, cmd=str(command)
        ) as workunit:
