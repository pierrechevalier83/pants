...
class NpmResolver(Subsystem, NodeResolverBase):
    ...
    """
        if self.get_options().force_option_override:
            ...
            install_optional = (
                install_optional
                if install_optional is not None
                else self.get_options().install_optional
            )
            production_only = (
                production_only
                if production_only is not None
                else self.get_options().install_production
            )
            force = force if force is not None else self.get_options().force
            frozen_lockfile = (
                frozen_lockfile
                if frozen_lockfile is not None
                else self.get_options().frozen_lockfile
            )
                ...
                for package_name, file_path in source_deps.items():
                    if self._get_target_from_package_name(target, package_name, file_path) is None:
                        raise TaskError(
                            ...
                            'Check your BUILD file for missing dependencies. ["{}": {}]'.format(
                ...
                if result != 0:
                    raise TaskError(
                        "Failed to resolve dependencies for {}:\n\t{} failed with exit code {}".format(
                            target.address.reference(), command, result
                        )
                    )
                if source_deps:
                    self._link_source_dependencies(
                        node_task, target, results_dir, node_paths, source_deps
                    )
    ...
    def _emit_package_descriptor(node_task, target, results_dir, node_paths):
        dependencies = {
            dep.package_name: node_paths.node_path(dep)
            if node_task.is_node_module(dep)
            else dep.version
        ...
        if "name" not in package:
            ...
            raise TaskError(
                "Package name in the corresponding package.json is not the same "
        ...
        node_task.context.log.debug(f"Adding {dependencies} to package.json for {package['name']}")
