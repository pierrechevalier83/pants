...
class TestNodeBuild(TaskTestBase):
    ...
    def test_node_module(self):
        ...
        target = self.make_target(
            spec=target_name, target_type=NodeModule, dependencies=[node_dependent_module],
        )
        ...
        node_dependent_paths = self._get_all_bundleable_js_path(
            bundleable_js, node_dependent_module
        )
        ...
        self.assertEqual(
            os.path.normpath(target_paths[0]), os.path.normpath(node_paths.node_path(target))
        )
        ...
        self.assertEqual(
            os.path.realpath(
                os.path.join(node_dependent_classpath[0][1], node_dependent_module_name)
            ),
    ...
    def test_node_module_no_artifacts(self):
        node_dependent_module = self.make_target(
            spec=":dependent_target", target_type=NodeModule, dev_dependency=True
        )
    ...
    def test_run_build_script(self):
      ...
      """
        ...
        self.assertEqual(set(os.listdir(target_paths[0])), {"output_file"})
