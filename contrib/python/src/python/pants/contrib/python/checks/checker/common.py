...
class OffByOneList(Sequence):
    ...
    def __getslice(self, sl):
        if sl.start == 0 or sl.stop == 0:
            raise IndexError
        new_slice = slice(
            sl.start - 1 if sl.start > 0 else sl.start, sl.stop - 1 if sl.stop > 0 else sl.stop
        )
...
class PythonFile(object):
    ...
    def __init__(self, blob, tree, root, filename):
        ...
        self.logical_lines = dict(
            (start, (start, stop, indent))
            for start, stop, indent in self.iter_logical_lines(self._blob)
        )

    @property
    def path(self):
        return os.path.join(self._root, self.filename) if self._root else self.filename
...
class Nit(object):
    ...
    SEVERITY = {COMMENT: "COMMENT", WARNING: "WARNING", ERROR: "ERROR"}
    ...
    def __init__(self, code, severity, filename, message, line_range=None, lines=None):
        if severity not in self.SEVERITY:
            raise ValueError(
                "Severity should be one of {}".format(" ".join(self.SEVERITY.values()))
            )
...
class CheckstylePlugin(object):
    ...
    def nit(self, code, severity, message, line_number_or_ast=None):
        ...
        return Nit(
            code, severity, self.python_file.filename, message, line_range=line_range, lines=lines
        )
