...
from pants.contrib.python.checks.checker.import_order import ImportOrder, ImportType
...
class ImportOrderTest(CheckstylePluginTestBase):
    ...
    def test_classify_import_chunks(self):
        ...
        self.assertEqual(
            (ast.ImportFrom, ast.ImportFrom), self.get_import_chunk_types(ImportType.TWITTER)
        )
        self.assertEqual((ast.ImportFrom,), self.get_import_chunk_types(ImportType.GEN))
        self.assertEqual((ast.ImportFrom,), self.get_import_chunk_types(ImportType.PACKAGE))
        self.assertEqual(
            (ast.ImportFrom, ast.Import), self.get_import_chunk_types(ImportType.THIRD_PARTY)
        )
