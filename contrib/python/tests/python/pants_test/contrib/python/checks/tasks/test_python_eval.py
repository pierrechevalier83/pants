...
class PythonEvalTest(PythonTaskTestBase):
    ...
    def _create_graph(self, broken_c_library=True):
        ...
        self.d_library = self.create_python_library(
            "src/python/d",
            "d",
            {
                "d.py": dedent(
        ...
        self.e_binary = self.create_python_binary(
            "src/python/e", "e", "a.a", dependencies=["//src/python/a"]
        )
        self.f_binary = self.create_python_binary(
            "src/python/f", "f", "a.a:compile_time_check_decorator", dependencies=["//src/python/a"]
        )
        self.g_binary = self.create_python_binary(
            "src/python/g", "g", "a.a:does_not_exist", dependencies=["//src/python/a"]
        )
        self.h_binary = self.create_python_binary("src/python/h", "h", "a.a")

    def _create_task(self, target_roots, options=None):
        # Until the --closure options is removed, we must explicitly set --transitive
        ...
        return self.create_task(
            self.context(target_roots=target_roots, for_subsystems=[PythonSetup, PythonRepos])
        )
