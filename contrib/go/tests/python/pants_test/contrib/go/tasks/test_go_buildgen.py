...
class GoBuildgenTest(TaskTestBase):
    ...
    def test_existing_targets_wrong_type(self):
        ...
        with self.assertRaises(task.GenerationError) as exc:
            task.execute()
        self.assertEqual(
            GoTargetGenerator.WrongLocalSourceTargetTypeError, type(exc.exception.cause)
        )
    ...
    def test_stitch_deps_generate_builds_custom_extension(self):
        ...
        self.assertEqual(
            {"src/go/src/fred/BUILD.gen", "src/go/src/jane/BUILD.gen"},
    ...
    def test_stitch_deps_remote_existing_rev_respected(self):
        self.make_target(
            "3rdparty/go/pantsbuild.org/fake:prod", GoRemoteLibrary, pkg="prod", rev="v1.2.3"
        )
        ...
        self.assertEqual(
            {"src/go/src/jane/BUILD", "3rdparty/go/pantsbuild.org/fake/BUILD"},
            self.buildroot_files() - pre_execute_files,
        )
