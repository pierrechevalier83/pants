...
class GoFetch(GoTask):
    ...
    def _log_undeclared_deps(self, undeclared_deps):
        for dependee, deps in undeclared_deps.items():
            self.context.log.error(
                "{address} has remote dependencies which require local declaration:".format(
                    address=dependee.address.reference()
                )
            )
            for dep_import_path, address in deps:
                self.context.log.error(
                    "\t--> {import_path} (expected go_remote_library declaration "
                    "at {address})".format(import_path=dep_import_path, address=address.reference())
                )
            ...
            with temporary_dir() as tmp_fetch_root:
                with self.context.new_workunit(f"fetch {pkg}"):
                    ...
                    for path in os.listdir(tmp_fetch_root):
                        shutil.move(
                            os.path.join(tmp_fetch_root, path), os.path.join(root_dir, path)
                        )
    ...
    def _map_fetched_remote_source(
        self,
        go_remote_lib,
        gopath,
        ...
        if os.path.exists(remote_import_paths_cache):
            ...
            remote_import_paths = self._get_remote_import_paths(
        ...
        for remote_import_path in remote_import_paths:
            ...
            if not any(address == lib.address for lib in all_known_remote_libs):
                try:
                    ...
                    same_remote_libs = [
                        lib for lib in all_known_remote_libs if spec_path == lib.address.spec_path
                    ]
                    ...
                    remote_lib = self._resolve(
                        go_remote_lib, address, package_path, rev, implicit_ok
                    )
    ...
    """
        ...
        with self.invalidated(go_remote_libs) as invalidation_check:
            ...
            for vt in invalidation_check.all_vts:
                ...
                self._map_fetched_remote_source(
                    go_remote_lib,
                    gopath,
                    all_known_remote_libs,
                    resolved_remote_libs,
                ...
                go_remote_lib_src[go_remote_lib] = os.path.join(
                    gopath, "src", go_remote_lib.import_path
                )
        ...
        trans_undeclared_deps = self._transitive_download_remote_libs(
            resolved_remote_libs, all_known_remote_libs
        )
    ...
    """
        ...
        return [
            imp
            for imp in import_listing.imports
            if (
                not self.import_oracle.is_go_internal_import(imp)
                and
        ...
        if os.path.exists(path):
            with open(path, "r") as fp:
                return dict(
                    {
                        import_path: root
                        for import_path, root in (x.strip().split("\t") for x in fp.readlines())
                    }
                )
        else:
            return {}
