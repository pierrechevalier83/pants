...
class ImportOracle:
    ...
    class ImportListing(
        namedtuple("ImportListing", ["pkg_name", "imports", "test_imports", "x_test_imports"])
    ):
    ...
    """
        ...
        with self._workunit_factory(
            f"list {pkg}", cmd=str(go_cmd), labels=[WorkUnitLabel.TOOL]
        ) as workunit:
            ...
            if returncode != 0:
                raise self.ListDepsError(
                    "Problem listing imports for {}: {} failed with exit code {}".format(
                        pkg, go_cmd, returncode
                    )
                )
