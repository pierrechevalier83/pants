                        ...
                        try:
                            ...
                            if not self._generate_remotes:
                                raise self.NewRemoteEncounteredButRemotesNotAllowedError(
                                    "Cannot generate dependency for remote import path {}".format(
                                        import_path
                                    )
                                )
                            self._build_graph.inject_synthetic_target(
...
class GoBuildgen(GoTask):
    ...
    class TemplateResult(
        ...
        def __str__(self):
            ...
            return "{build_file_path} ({import_paths}){rev}".format(
                build_file_path=self.build_file_path, import_paths=import_paths, rev=rev
            )
