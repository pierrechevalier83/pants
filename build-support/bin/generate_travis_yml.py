...
def bootstrap_linux(python_version: PythonVersion) -> Dict:
    ...
    shard["env"] = shard.get("env", []) + _bootstrap_env(
        python_version=python_version, platform=Platform.linux
    )
    return shard
...
def bootstrap_osx(python_version: PythonVersion) -> Dict:
    ...
    shard["env"] = shard.get("env", []) + _bootstrap_env(
        python_version=python_version, platform=Platform.osx
    )
    return shard
...
def integration_tests_v2(python_version: PythonVersion) -> Dict:
    ...
    shard["env"] = shard.get("env", []) + [f"CACHE_NAME=integration.v2.py{python_version.number}"]
    return shard
...
def osx_platform_tests(python_version: PythonVersion) -> Dict:
    ...
    shard["env"] = shard.get("env", []) + [
        f"CACHE_NAME=osx_platform_tests.py{python_version.number}"
    ]
    return shard
...
def _osx_sanity_check(
    ...
    shard["env"] = shard.get("env", []) + [
        f"CACHE_NAME=osx_sanity.10_{os_version_number}.py{python_version.number}"
    ]
    return shard
