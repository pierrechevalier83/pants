...
class AllTheThingsTestCase(unittest.TestCase):
    ...
    def test_fixup_internal_links(self):
        ...
        html = sitegen.render_html(
            "index", self.config, self.soups, self.precomputed, TEMPLATE_MUSTACHE
        )
        self.assertIn("subdir/page1.html", html, "p1.html link did not get fixed up to page1.html")

    def test_pantsrefs(self):
        sitegen.link_pantsrefs(self.soups, self.precomputed)
        p1_html = sitegen.render_html(
            "subdir/page1", self.config, self.soups, self.precomputed, TEMPLATE_MUSTACHE
        )
        self.assertIn(
            'href="../index.html#pantsmark_index"', p1_html, "pantsref_index did not get linked"
        )
        p2_html = sitegen.render_html(
            "subdir/page2", self.config, self.soups, self.precomputed, TEMPLATE_MUSTACHE
        )
        self.assertIn('href="page1.html#an_pantsmark"', p2_html, "pantsref_p1 did not get linked")

    def test_find_title(self):
        p2_html = sitegen.render_html(
            "subdir/page2", self.config, self.soups, self.precomputed, "{{title}}"
        )
        self.assertEqual(p2_html, "Page 2: Electric Boogaloo", """Didn't find correct title""")
        # ascii worked? great, try non-ASCII
        p1_html = sitegen.render_html(
            "subdir/page1", self.config, self.soups, self.precomputed, "{{title}}"
        )
        self.assertEqual(p1_html, u"東京 is Tokyo", """Didn't find correct non-ASCII title""")
    ...
    def test_site_toc(self):
        ...
        rendered_expected = (
            ...
            "{{{q}link{q}: {q}index.html{q}, {q}text{q}: {q}Pants Build System{q}, {q}here{q}: True}}] HEADING=collapse".format(
                q=escaped_single_quote
            )
        )
    ...
    def test_transform_fixes_up_internal_links(self):
        ...
        self.assertTrue(
            "subdir/page1.html" in html, "p1.html link did not get fixed up to page1.html"
        )
