    ...
    def _ensure_bool(s):
        if isinstance(s, str):
            if s.lower() == "true":
                ...
                raise Parser.BooleanConversionError(
                    'Got "{0}". Expected "True" or "False".'.format(s)
                )
        if s is True:
            return True
        elif s is False:
            ...
            def add_flag_val(v):
                if v is None:
                    if implicit_value is None:
                        raise ParseError(
                            "Missing value for command line flag {} in {}".format(
                                arg, self._scope_str()
                            )
                        )
                    else:
            ...
            if val.rank > RankedValue.HARDCODED:
                ...
                if len(mutex_map[dest]) > 1:
                    raise self.MutuallyExclusiveOptionError(
                        "Can only provide one of the mutually exclusive options {}".format(
                            mutex_map[dest]
                        )
                    )
