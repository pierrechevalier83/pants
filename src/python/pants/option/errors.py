...
class RegistrationError(OptionsError):
    ...
    def __init__(self, msg, scope, option):
        super().__init__(
            "{} [option {} in {}].".format(
                msg, option, "global scope" if scope == GLOBAL_SCOPE else "scope {}".format(scope)
            )
        )
...
FrozenRegistration = mk_registration_error(
...
InvalidKwargNonGlobalScope = mk_registration_error(
...
MemberTypeNotAllowed = mk_registration_error(
...
OptionAlreadyRegistered = mk_registration_error(
...
RecursiveSubsystemOption = mk_registration_error(
