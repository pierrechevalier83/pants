  ...
  """
    ...
    def __init__(self, known_scope_infos):
        ...
        self._known_scoping_prefixes = [
            ("{0}-".format(si.scope.replace(".", "-")), si) for si in sorted_scope_infos
        ]
    ...
    """
        ...
        if self._unconsumed_args and self._unconsumed_args[-1] == "goal":
            ...
            print(
                "WARNING: Specifying 'goal' explicitly is no longer necessary, and deprecated.",
                file=sys.stderr,
            )
            self._unconsumed_args.pop()
        ...
        return SplitArgs(
            ...
            targets,
            passthru,
            passthru_owner if passthru else None,
            self._unknown_scopes,
        )
