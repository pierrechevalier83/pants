...
class PailgunServer(ThreadingMixIn, TCPServer):
    ...
    def ensure_request_is_exclusive(self, environment, request):
        ...
        def yield_and_release(time_waited):
            try:
                self.logger.debug(
                    "request lock acquired {}.".format(
                        "on the first try"
                        if time_waited == 0
                        else "in {} seconds".format(time_waited)
                    )
                )
                yield
            finally:
        ...
        if self.free_to_handle_request_lock.acquire(timeout=0):
            ...
            self.logger.debug(
                "request {} didn't aquire the lock on the first try, polling...".format(request)
            )
    ...
    def process_request_thread(self, request, client_address):
        ...
        try:
            ...
            self.logger.error(
                "Request {} abruptly closed with {}, probably because the client crashed or was sent a SIGINT.".format(
                    request, type(e)
                )
            )
