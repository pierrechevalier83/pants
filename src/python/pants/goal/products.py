...
class UnionProducts:
    ...
    def __eq__(self, other):
        return (
            isinstance(other, UnionProducts)
            and self._products_by_target == other._products_by_target
        )
...
class Products:
    ...
    def get_only(self, product_type, target):
        ...
        if len(product_mapping) != 1:
            raise ProductError(
                "{} directories in product mapping: requires exactly 1.".format(
                    len(product_mapping)
                )
            )
        ...
        for _, files in product_mapping.items():
            if len(files) != 1:
                raise ProductError(
                    "{} files in target directory: requires exactly 1.".format(len(files))
                )
