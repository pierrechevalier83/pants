...
class LocalPantsRunner(ExceptionSink.AccessGlobalExiterMixin):
    ...
    @classmethod
    def create(
        cls, args, env, target_roots=None, daemon_graph_session=None, options_bootstrapper=None
    ):
    ...
    """
        ...
        target_roots = cls._maybe_init_target_roots(
            target_roots, graph_session, options, build_root
        )
    ...
    def set_start_time(self, start_time):
        ...
        target_specs = [
            spec_parser.parse_spec(spec).to_spec_string() for spec in self._options.positional_args
        ]
        # Note: This will not include values from `--owner-of` or `--changed-*` flags.
        self._run_tracker.run_info.add_info(
            "specs_from_command_line", target_specs, stringify=False
        )
    ...
    def _maybe_run_v2(self):
        ...
        return self._graph_session.run_console_rules(
            self._options_bootstrapper, self._options, goals, self._target_roots,
        )
    ...
    def _run(self):
        ...
        try:
            ...
            streaming_reporter = StreamingWorkunitHandler(
                self._scheduler_session, callbacks=callbacks
            )
