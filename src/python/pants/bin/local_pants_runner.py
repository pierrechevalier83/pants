...
class LocalPantsRunner(ExceptionSink.AccessGlobalExiterMixin):
    ...
    def _maybe_init_graph_session(graph_session, options_bootstrapper, build_config, options):
        if not graph_session:
            ...
            graph_session = graph_scheduler_helper.new_session(
                zipkin_trace_v2, RunTracker.global_instance().run_id, v2_ui
            )
    ...
    def set_start_time(self, start_time):
        ...
        target_specs = [
            spec_parser.parse_spec(spec).to_spec_string() for spec in self._options.target_specs
        ]
    ...
    def _maybe_run_v2(self):
        ...
        return self._graph_session.run_console_rules(
            self._options_bootstrapper, goals, self._target_roots,
        )
