...
class TargetRootsCalculator:
    ...
    """
        ...
        dependencies = tuple(
            OrderedSet(spec_parser.parse_spec(spec_str) for spec_str in target_specs)
        )
        return Specs(
    ...
    """
        ...
        targets_specified = sum(
            1
            for item in (changed_request.is_actionable(), owned_files, spec_roots.dependencies)
            if item
        )
        ...
        if changed_request.is_actionable():
            ...
            changed_files = cls.changed_files(
                scm, changes_since=changed_request.changes_since, diffspec=changed_request.diffspec
            )
            ...
            return TargetRoots(
                Specs(dependencies=dependencies, exclude_patterns=exclude_patterns, tags=tags)
            )
