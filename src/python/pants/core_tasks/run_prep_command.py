  ...
  """
    ...
    def execute(self):
        ...
        with self.context.new_workunit(
            name="prep_command", labels=[WorkUnitLabel.PREP]
        ) as workunit:
            ...
            for item in cmdlines:
                ...
                if not cmdline in completed_cmdlines:
                    ...
                    try:
                        ...
                        raise TaskError(
                            "RunPrepCommand failed to execute {cmdline}: {error}".format(
                    ...
                    workunit.set_outcome(
                        WorkUnit.FAILURE if process.returncode else WorkUnit.SUCCESS
                    )
                    if process.returncode:
                        raise TaskError(
                            "RunPrepCommand failed to run {cmdline}".format(cmdline=cmdline)
                        )
