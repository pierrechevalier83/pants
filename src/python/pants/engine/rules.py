...
class _RuleVisitor(ast.NodeVisitor):
    ...
    def _generate_ast_error_message(self, node, msg):
        ...
        tokenized_rule_body = asttokens.ASTTokens(
            self._func_source, tree=self._func_node, filename=filename
        )
        start_offset, _ = tokenized_rule_body.get_text_range(node)
        line_offset, col_offset = asttokens.LineNumbers(self._func_source).offset_to_line(
            start_offset
        )
            ...
            if self._stmt_is_at_end_of_parent_list(expr_for_yield):
                if self._is_get(node.value):
                    raise self.YieldVisitError(
                        self._generate_ast_error_message(
                            node,
                            dedent(
                                """\
...
def _ensure_type_annotation(
    ...
    if not isinstance(annotation, type):
        raise raise_type(
            f"The annotation for {name} must be a type, "
...
def union(cls):
    ...
    return type(
        cls.__name__,
        (cls,),
        {"_is_union": True, "non_member_error_message": non_member_error_message,},
    )
