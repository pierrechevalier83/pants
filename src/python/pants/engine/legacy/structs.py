...
class BaseGlobs(Locatable, metaclass=ABCMeta):
    ...
    def from_sources_field(sources, spec_path):
        ...
        elif isinstance(sources, (MutableSet, MutableSequence, tuple)) and all(
            isinstance(s, str) for s in sources
        ):
            return Files(*sources, spec_path=spec_path)
        else:
            raise ValueError(
                "Expected either a glob or list of literal sources: got: {}".format(sources)
            )
    ...
    def _filespec_for_exclude(raw_exclude, spec_path):
        if isinstance(raw_exclude, str):
            raise ValueError(
                'Excludes of type `{}` are not supported: got "{}"'.format(
                    type(raw_exclude).__name__, raw_exclude
                )
            )
    ...
    def __init__(self, *patterns, **kwargs):
        ...
        self._excluded_file_globs = self._filespec_for_exclude(raw_exclude, raw_spec_path).get(
            "globs", []
        )
    ...
    def _gen_init_args_str(self):
        ...
        keyword_args = ", ".join(["{}={}".format(k, repr(v)) for k, v in self._kwargs.items()])
        if keyword_args:
