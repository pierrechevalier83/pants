...
class LintTest(TestBase):
    ...
    def run_lint_rule(
        *,
        ...
        if mock_linters is None:
            mock_linters = lambda target_adaptor: LintResults(
                [LintResult(exit_code=1, stdout=f"Linted `{target_adaptor.name}`", stderr="")]
            )
        ...
        result: Lint = run_rule(
            ...
            mock_gets=[
                MockGet(
                    product_type=LintResults, subject_type=PythonTargetAdaptor, mock=mock_linters
                ),
            ],
        )
    ...
    def test_multiple_targets_with_one_linter(self) -> None:
        ...
        result, stdout = self.run_lint_rule(
            targets=[
                FmtTest.make_hydrated_target(name="good"),
                FmtTest.make_hydrated_target(name="bad"),
            ],
