...
def fast_test(console: Console, addresses: BuildFileAddresses) -> Test:
        ...
        if test_result.stderr:
            ...
            console.write_stdout(
                "{} stderr:\n{}\n".format(
                    address.reference(),
                    (
                        console.red(test_result.stderr)
                        if test_result.status == Status.FAILURE
    ...
    for address, test_result in filtered_results:
        console.print_stdout(
            "{0:80}.....{1:>10}".format(address.reference(), test_result.status.value)
        )
...
def coordinator_of_tests(
    target: HydratedTarget, union_membership: UnionMembership, provenance_map: AddressProvenanceMap
    ...
    if provenance_map.is_single_address(target.address) or union_membership.is_member(
        TestTarget, target.adaptor
    ):
...
def rules():
    return [coordinator_of_tests, fast_test]
