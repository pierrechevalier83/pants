...
class ClocTest(ConsoleRuleTestBase):
    ...
    def assert_counts(
        self, result: Sequence[str], lang: str, num_files: int, blank: int, comment: int, code: int
    ) -> None:
        for line in result:
            fields = line.split()
    ...
    def test_cloc(self) -> None:
        ...
        self.add_to_build_file(
            "src/py/foo",
            'python_library(dependencies=["src/py/dep"], sources=["foo.py", "bar.py"])',
        )
        ...
        output = self.execute_rule(
            args=["src/py/foo", "src/java/foo", "--cloc2-no-transitive"]
        ).splitlines()
