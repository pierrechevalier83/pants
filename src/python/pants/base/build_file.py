...
class BuildFile(ABC):
    ...
    def scan_build_files(project_tree, base_relpath, build_ignore_patterns=None):
        ...
        if base_relpath and not project_tree.isdir(base_relpath):
            raise BuildFile.BadPathError(
                "Can only scan directories and {0} is not a valid dir.".format(base_relpath)
            )
        ...
        for root, dirs, files in project_tree.walk(base_relpath or "", topdown=True):
            excluded_dirs = list(
                build_ignore_patterns.match_files(
                    "{}/".format(os.path.join(root, dirname)) for dirname in dirs
                )
            )
    ...
    def _build_files_from_paths(project_tree, rel_paths, build_ignore_patterns):
        if build_ignore_patterns:
            build_files_without_ignores = rel_paths.difference(
                build_ignore_patterns.match_files(rel_paths)
            )
        else:
        ...
        return OrderedSet(
            sorted(
                (
                    BuildFile._cached(project_tree, relpath)
                    for relpath in build_files_without_ignores
                ),
