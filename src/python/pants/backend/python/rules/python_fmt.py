...
@rule
def get_black_input(wrapped_target: FormattablePythonTarget, black: Black,) -> BlackInput:
    ...
    resolved_requirements_pex = yield Get(
        ...
        CreatePex(
            ...
            interpreter_constraints=PexInterpreterConstraints(
                constraint_set=frozenset(black.default_interpreter_constraints)
            ),
    ...
    merged_input_files = yield Get(
        Digest, DirectoriesToMerge(directories=tuple(all_input_digests)),
    )
    yield BlackInput(config_path, resolved_requirements_pex, merged_input_files)


def _generate_black_pex_args(
    files: Set[str], config_path: str, *, check_only: bool
) -> Tuple[str, ...]:
...
def _generate_black_request(
    ...
    *,
    ...
    target = wrapped_target.target
    pex_args = _generate_black_pex_args(
        target.sources.snapshot.files, black_input.config_path, check_only=check_only
    )
...
def fmt_with_black(
    ...
    request = _generate_black_request(
        wrapped_target, black_input, python_setup, subprocess_encoding_environment, check_only=False
    )
...
def lint_with_black(
    ...
    request = _generate_black_request(
        wrapped_target, black_input, python_setup, subprocess_encoding_environment, check_only=True
    )

    result = yield Get(FallibleExecuteProcessResult, ExecuteProcessRequest, request)

    yield LintResult(
        exit_code=result.exit_code, stdout=result.stdout.decode(), stderr=result.stderr.decode(),
    )
