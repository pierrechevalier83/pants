...
async def create_isort_request(
    ...
    sources_digest = (
        wrapped_target.prior_formatter_result_digest or target.sources.snapshot.directory_digest
    )
...
async def fmt(wrapped_target: IsortTarget, isort_setup: IsortSetup) -> FmtResult:
    args = IsortArgs.create(
        wrapped_target=wrapped_target, isort_setup=isort_setup, check_only=False
    )
