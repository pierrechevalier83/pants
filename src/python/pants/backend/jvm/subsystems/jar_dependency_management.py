...
class JarDependencyManagement(Subsystem):
    ...
    def resolve_version_conflict(self, managed_coord, direct_coord, force=False):
        ...
        if strategy == "USE_DIRECT_IF_FORCED":
            ...
            if force:
                log(
                    "[{}] Using direct version, because force=True: {}".format(
                ...
                log(
                    "[{}] Using managed version, because force=False: {}".format(
        ...
        if strategy == "USE_NEWER":
            newer = max(
                [managed_coord, direct_coord], key=lambda coord: Revision.lenient(coord.rev)
            )
            log(message)
    ...
    def put(self, artifact):
        ...
        if artifact.rev is None:
            raise self.MissingVersion(
                'Cannot pin an artifact to version "None"! {}'.format(artifact)
            )
...
class JarDependencyManagementSetup(Task):
    ...
    class MissingVersion(TargetDefinitionException):
        ...
        def __init__(self, target, coord):
            super(JarDependencyManagementSetup.MissingVersion, self).__init__(
                target,
                "The jar {} specified in {} is missing a version (rev).".format(
                    coord, target.address.spec
                ),
            )
    ...
    def execute(self):
        ...
        targets = set(
            self.context.targets(predicate=lambda t: isinstance(t, ManagedJarDependencies))
        )
        ...
        if not isinstance(target, ManagedJarDependencies):
            if not any(isinstance(t, ManagedJarDependencies) for t in target.closure()):
                raise self.InvalidDefaultTarget(
                    ...
                    'managed_jar_dependencies() target! "{}" is a {}.'.format(
                        spec, type(target).__name__
                    )
                )
    ...
    def _compute_artifact_sets(self, targets):
        ...
        for target in targets:
            ...
            self.context.log.debug(
                "Computed artifact map for {} ({} jars)".format(
                    target.id, dm._artifact_set_map[target.id]
                )
            )
