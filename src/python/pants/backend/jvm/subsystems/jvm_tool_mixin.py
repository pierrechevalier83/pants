...
class JvmToolMixin:
    ...
    def register_jvm_tool(
        ...
        def formulate_help():
            if classpath:
                ...
                return "Target address spec for specifying the classpath of the {} jvm tool.".format(
                    key
                )
    ...
    def tool_classpath_entries_from_products(products, key, scope):
        ...
        if not callback:
            raise TaskError(
                "No bootstrap callback registered for {key} in {scope}".format(key=key, scope=scope)
            )
        return callback()
