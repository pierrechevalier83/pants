...
class Jacoco(CoverageEngine):
    ...
    def run_modifications(self, output_dir):
        ...
        agent_option = "-javaagent:{agent}=destfile={destfile}".format(
            agent=self._agent_path, destfile=datafile
        )
    ...
    def report(self, output_dir, execution_failed_exception=None):
        ...
        for report_format in ("xml", "csv", "html"):
            ...
            args = (
                ...
                + [
                    "--{report_format}={target_path}".format(
                        report_format=report_format, target_path=target_path
                    )
                ]
            )
    ...
    def _get_jacoco_coverage_targets(self, targets):
        ...
        if len(coverage_targets) == 0:
            raise TaskError(
                "No coverage target specs matched jacoco report filters ({0})".format(
