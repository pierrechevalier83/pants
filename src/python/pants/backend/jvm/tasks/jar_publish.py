...
class PomWriter:
    ...
    def _jardep(self, jar):
        return TemplateData(
            ...
            scope="compile",
            excludes=[
                TemplateData(org=exclude.org, name=exclude.name)
...
class JarPublish(TransitiveOptionRegistrar, HasTransitiveOptionMixin, ScmPublishMixin, JarTask):
    ...
    * ``--restrict-push-branches`` - Optional list of branches to restrict publishing to.
            ...
            if len(components) == 2:
                ...
                try:
                    ...
                    if not target:
                        ...
                        prompt = (
                            "did you mean" if len(siblings) == 1 else "maybe you meant one of these"
                        )
                        raise TaskError(
                            "{} => {}?:\n    {}".format(
                    ...
                    raise TaskError(
                        "{message}\n  Problem identifying target at {spec}".format(
                            message=e, spec=spec
                        )
                    )
    ...
    def confirm_push(self, coord, version):
        ...
        if not isatty:
            return True
        push = input("\nPublish {} with revision {} ? [y|N] ".format(coord, version))
        print("\n")
    ...
    def execute(self):
        ...
        def get_db(tgt):
            ...
            if tgt.provides is None:
                raise TaskError(
                    "trying to publish target {!r} which does not provide an artifact".format(tgt)
                )
            ...
            if not result:
                ...
                try:
                    ...
                    raise TaskError(
                        "Repository {0} has no entry in the --repos option.".format(
        ...
        for target in exported_targets:
            ...
            if self.publish_changelog:
                if no_changes:
                    ...
                    changelog = (
                        self.changelog(target, oldentry.sha) or "Direct dependencies changed.\n"
                    )
            ...
            tag_name = (
                "{org}-{name}-{rev}".format(org=org, name=name, rev=rev) if self.commit else None
            )
            ...
            if no_changes and not self.force:
                ...
                print(
                    "Skipping {} to resume at {}".format(
                        jar_coordinate(
                            jar,
                            (newentry.version() if self.force else oldentry.version()).version(),
                        ),
                ...
                if self.dryrun:
                    print(
                        "Skipping publish of {0} in test mode.".format(
                            pushdb_coordinate(jar, newentry)
                        )
                    )
                else:
                    self.publish(
                        publications, jar=jar, entry=newentry, repo=repo, published=published
                    )
                    ...
                    if self.commit:
                        ...
                        self.publish_pushdb_changes_to_remote_scm(
                            ...
                            tag_message="Publish of {coordinate} initiated by {user} {cause}".format(
                                ...
                                cause="with forced revision"
                                if (org, name) in self.overrides
                                else "(autoinc)",
                            ),
    ...
    def check_for_duplicate_artifacts(self, targets):
        ...
        def duplication_message(artifact):
            ...
            return "\n  {artifact} is defined by:{specs}".format(
                artifact=artifact, specs="".join(specs)
            )
    ...
    def check_targets(self, targets):
        ...
        for target in targets:
            target.walk(
                functools.partial(collect_invalid, target),
        ...
        if invalid:
            ...
            raise TaskError(
                "The following errors must be resolved to publish.{}".format("".join(msg))
            )
    ...
    def generate_ivy(self, jar, version, publications):
        ...
        generator = Generator(
            template_text, org=jar.org, name=jar.name, rev=version, publications=pubs
        )
