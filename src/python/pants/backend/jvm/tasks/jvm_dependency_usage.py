...
class JvmDependencyUsage(Task):
  ...
  """
    ...
    def register_options(cls, register):
        ...
        register(
            "--output-file", type=str, help="Output destination. When unset, outputs to <stdout>."
        )
        register(
            "--use-cached",
            type=bool,
    ...
    def create_dep_usage_node(self, target, targets_by_file, transitive_deps):
        ...
        for dep_tgt, aliased_from in declared_deps_with_aliases:
            ...
            if self._select(derived_from):
                node.add_edge(
                    _construct_edge(dep_tgt, products_used=set()), derived_from, aliased_from
                )
...
class DependencyUsageGraph:
    ...
    def _used_ratio(self, dep_tgt, edge):
        if edge.products_used:
            ...
            dep_tgt_products_total = (
                self._nodes[dep_tgt].products_total if dep_tgt in self._nodes else 1
            )
