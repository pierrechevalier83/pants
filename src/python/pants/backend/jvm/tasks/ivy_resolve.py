...
class IvyResolve(IvyTaskMixin, NailgunTask):
    ...
    def register_options(cls, register):
        ...
        register("--report", type=bool, help="Generate an ivy resolve html report")
        register(
            "--open",
            type=bool,
        ...
        register("--args", type=list, fingerprint=True, help="Pass these extra args to ivy.")
        register(
            "--confs",
            type=list,
            default=["default"],
        ...
        cls.register_jvm_tool(
            register, "xalan", classpath=[JarDependency(org="xalan", name="xalan", rev="2.7.1"),]
        )
    ...
    def _generate_ivy_report(self, result):
        ...
        for conf in self.get_options().confs:
            ...
            out = os.path.join(
                self._outdir, "{org}-{name}-{conf}.html".format(org=org, name=name, conf=conf)
            )
