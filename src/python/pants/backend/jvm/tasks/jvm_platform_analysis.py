...
class JvmPlatformAnalysisMixin:
    ...
    """
        ...
        return {
            target: deps
            for target, deps in jvm_deps.items()
...
class JvmPlatformExplain(JvmPlatformAnalysisMixin, ConsoleTask):
  ...
  """
    ...
    def dependency_map(self):
        ...
        return {
            target: deps
            for target, deps in full_map.items()
    ...
    def possible_version_evaluation(self):
        ...
        for target in sorted(filter(self._is_relevant, self.jvm_targets)):
            ...
            if min_version and max_version:
                ...
                range_text = "*"
            yield "{address}: {range}  (is {current})".format(
                address=target.address.spec, range=range_text, current=current_text,
            )
            if self.get_options().detailed or not current_valid:
                if min_version:
                    min_because = [
                        t
                        for t in ranges.target_dependencies[target]
                ...
                if max_version:
                    max_because = [
                        t
                        for t in ranges.target_dependees[target]
    ...
    def _changeable(self, change_name, can_change, change_getter):
        ...
        for target, allowed in sorted(changes.items()):
            yield "{target} can {change} to {allowed}".format(
                target=target.address.spec, allowed=allowed or "*", change=change_name
            )
        yield ""
