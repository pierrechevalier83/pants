...
class ClasspathProducts:
        ...
        for target, classpath_entries_for_target in target_to_classpath.items():
            ...
            if len(classpath_entries_for_target) > 0:
                ...
                for (index, entry) in enumerate(classpath_entries_for_target):
                    ...
                    if not os.path.exists(real_entry_path):
                        raise MissingClasspathEntryError(
                            ...
                            "{src} into {dst}".format(
                                realpath=real_entry_path, src=entry.path, dst=symlink_path
                            )
                        )
                ...
                if save_classpath_file:
                    ...
                    with safe_open(
                        "{}classpath.txt".format(classpath_prefix_for_target), "w"
                    ) as classpath_file:
    ...
    def get_classpath_entries_for_targets(self, targets, respect_excludes=True):
        ...
        return list(
            OrderedSet(
                [
                    cp
                    for cp, target in self.get_product_target_mappings_for_targets(
    ...
    def update(self, other):
        ...
        if self._pants_workdir != other._pants_workdir:
            raise ValueError(
                "Other ClasspathProducts from a different pants workdir {}".format(
                    other._pants_workdir
                )
            )
