...
class BaseZincCompile(JvmCompile):
    ...
    def validate_arguments(log, whitelisted_args, args):
        ...
        def validate(idx):
            ...
            log.warn(
                "Zinc argument '{}' is not supported, and is subject to change/removal!".format(arg)
            )
            return 1
        ...
        if upstream_analysis:
            zinc_args.extend(
                [
                    "-analysis-map",
                    ",".join(
                        "{}:{}".format(relative_to_exec_root(k), relative_to_exec_root(v))
    ...
    def _compile_nonhermetic(self, jvm_options, ctx, classes_directory):
        ...
        self.context._scheduler.materialize_directories(
            (
                DirectoryToMaterialize(
                    get_buildroot(), self.post_compile_extra_resources_digest(ctx)
                ),
            )
        )
