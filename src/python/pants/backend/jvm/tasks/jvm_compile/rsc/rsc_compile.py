...
def _create_desandboxify_fn(possible_path_patterns):
    ...
    def desandboxify(path):
        ...
        for r in regexes:
            ...
            if match:
                logger.debug(
                    "path-cleanup: matched {} with {} against {}".format(match, r.pattern, path)
                )
...
class RscCompile(ZincCompile, MirroredTargetOptionMixin):
    ...
    def _compiler_tags(self):
        return {
            "{prefix}:{workflow_name}".format(
                prefix=self.get_options().force_compiler_tag_prefix, workflow_name=workflow.value
            ): workflow
    ...
    def register_options(cls, register):
        ...
        register(
            ...
            metavar="<workflow>",
            help="The default workflow to use to compile JVM targets. This is overriden on a per-target basis with the force-compiler-tag-prefix tag.",
            fingerprint=True,
        )
        ...
        register(
            ...
            help='The workflow to use to compile JVM targets, overriding the "workflow" option as well as any force-compiler-tag-prefix tags applied to targets. An example use case is to quickly turn off outlining workflows in case of errors.',
            fingerprint=True,
        )
    ...
    def _zinc_key_for_target(self, target, workflow):
        return workflow.match(
            {
                self.JvmCompileWorkflowType.zinc_only: lambda: "zinc[zinc-only]({})".format(
                    target.address.spec
                ),
                self.JvmCompileWorkflowType.zinc_java: lambda: "zinc[zinc-java]({})".format(
                    target.address.spec
                ),
                self.JvmCompileWorkflowType.rsc_and_zinc: lambda: "zinc[rsc-and-zinc]({})".format(
                    target.address.spec
                ),
                self.JvmCompileWorkflowType.outline_and_zinc: lambda: "zinc[outline-and-zinc]({})".format(
                    target.address.spec
                ),
            }
        )()
