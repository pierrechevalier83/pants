  ...
  """
    ...
    def execute(self):
        ...
        if requested_compiler == self.Compiler.ZINC and self.compiler_name == self.Compiler.RSC:
            ...
            self.context.log.debug(
                "Requested an unsupported compiler [{}], aborting".format(requested_compiler)
            )
            return
    ...
    def do_compile(self, invalidation_check, compile_contexts, classpath_product):
        ...
        with self.context.new_workunit(
            "isolation-{}-pool-bootstrap".format(self.name())
        ) as workunit:
    ...
    """
        ...
        if not cached_vts:
            ...
            return False
        assert cached_vts == [vts], "Cache returned unexpected target: {} vs {}".format(
            cached_vts, [vts]
        )
        self.context.log.info(
            "Hit cache during double check for {}".format(vts.target.address.spec)
        )
        return True
    ...
    def _record_target_stats(
        ...
        self.context.log.debug(
            "self.options_scope:{}, target:{}, stats_key:{}".format(
                self.options_scope, target, stats_key
            )
        )
        self.context.log.debug(
