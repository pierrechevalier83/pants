...
class GeneratePantsReference(Task):
  ...
  """
    ...
    def register_options(cls, register):
        register(
            "--pants-reference-template",
            default="reference/pants_reference.html",
            help="The template for the Pants reference document.  Defaults to generating "
            "a standalone reference page.",
        )
        register(
            "--build-dictionary-template",
            default="reference/build_dictionary.html",
    ...
    def _gen_reference(self):
        ...
        goal_scopes = sorted(
            [
                si.scope
                for si in self.context.options.known_scope_to_info.values()
        ...
        ref_page = self._do_render(
            self.get_options().pants_reference_template,
            {
    ...
    def _gen_build_dictionary(self):
        ...
        build_dict_page = self._do_render(
            self.get_options().build_dictionary_template,
            {"target_types": {"infos": target_type_infos}, "other_symbols": {"infos": other_infos}},
        )
