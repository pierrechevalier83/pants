...
class ApacheThriftGenBase(SimpleCodegenTask):
    ...
    def execute_codegen(self, target, target_workdir):
        ...
        for source in target.sources_relative_to_buildroot():
            ...
            with self.context.new_workunit(
                name=source, labels=[WorkUnitLabel.TOOL], cmd=" ".join(cmd)
            ) as workunit:
                result = subprocess.call(
                    cmd, stdout=workunit.output("stdout"), stderr=workunit.output("stderr")
                )
                if result != 0:
                    raise TaskError(
                        "{} ... exited non-zero ({})".format(self._thrift_binary, result)
                    )
    ...
    def _thrift_cmd(self):
        ...
        generator_spec = (
            "{}:{}".format(self.thrift_generator, ",".join(gen_opts))
            if gen_opts
