  ...
  ********************************************************
  ...
  """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.project_template = os.path.join(_TEMPLATE_BASEDIR, "project-12.mustache")
        self.workspace_template = os.path.join(_TEMPLATE_BASEDIR, "workspace-12.mustache")
        self.rootmodule_template = os.path.join(_TEMPLATE_BASEDIR, "rootmodule-12.mustache")
        ...
        with temporary_dir(root_dir=output_dir, cleanup=False) as output_project_dir:
            ...
            self.project_filename = os.path.join(
                self.gen_project_workdir, "{}.ipr".format(project_name)
            )
            self.workspace_filename = os.path.join(
                self.gen_project_workdir, "{}.iws".format(project_name)
            )
            self.rootmodule_filename = os.path.join(self.gen_project_workdir, "rootmodule.iml")
    ...
    def generate_project(self):
        ...
        abs_target_specs = [
            os.path.join(get_buildroot(), spec) for spec in self.context.options.positional_args
        ]
