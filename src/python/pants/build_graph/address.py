    ...
    def sanitize_path(cls, path):
        ...
        if components[-1].startswith("BUILD"):
            raise InvalidSpecPath(
                "Spec path {path} has {trailing} as the last path part and BUILD is "
                "reserved files".format(path=path, trailing=components[-1])
            )
        if os.path.isabs(path):
            raise InvalidSpecPath(
                "Spec has absolute path {path}; expected a path relative "
    ...
    def check_target_name(cls, spec_path, name):
        if not name:
            raise InvalidTargetName(
                "Spec {spec}:{name} has no name part".format(spec=spec_path, name=name)
            )
